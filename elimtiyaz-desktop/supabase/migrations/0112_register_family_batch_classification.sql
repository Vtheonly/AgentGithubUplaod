-- ============================================================================
-- 0112_register_family_batch_classification.sql
-- T-407 (85th session, 2026-09-22): the batch-registration classification
-- thread. The T-407 UI/integration suite of the three delivered academic
-- tasks (T-401/T-402/T-403) surfaced a REAL end-to-end gap: the wizard's
-- step 2 collects the filière/spécialité (0107's classification), the
-- wizard's submit maps it into CreateStudentInput — and then the value was
-- SILENTLY DROPPED on every path:
--
--   1. the desktop's Supabase batchRegister never put filière/specialité
--      into the register_family_batch student wire (studentWires), and
--   2. this RPC's jsonb_to_recordset column list never extracted them, so
--      even a wire that carried them would have lost them at the seam, and
--   3. upsert_student_from_import HAS carried the trailing p_filiere_code /
--      p_specialite_code params since 0107 — this RPC simply never passed
--      them (0102 predates 0107 and was never extended).
--
-- Net effect: a filière selected during batch registration never reached
-- the students row. The single createStudent path persisted it; the batch
-- path did not — a wire-contract divergence inside the SAME repository
-- file (the exact class of drift the one-business-path rule exists for).
--
-- THE FIX (the no-parallel-implementation rule): CREATE OR REPLACE with
-- the 0102 body VERBATIM plus exactly three changed lines — the two new
-- jsonb_to_recordset columns and the two trailing arguments threaded into
-- the canonical upsert (which owns the 'general' → NULL normalization,
-- the trimming and the catalog validation). Zero new SQL business logic.
--
-- The grants' type signature (uuid, jsonb, jsonb, jsonb, jsonb) is
-- UNCHANGED by the replace (same parameter list), so existing grants
-- survive; they are re-asserted anyway (idempotent, §15.34 narrow side).
--
...
-- ----------------------------------------------------------------------------
-- 1. The composite RPC — 0102 verbatim + the classification thread
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  --    to the upsert's trailing 0107 params — the upsert owns the
  --    'general' → NULL normalization and the catalog validation.
  -- ------------------------------------------------------------------
  FOR v_s IN
    SELECT *
...
-- ----------------------------------------------------------------------------
-- 2. Grants — re-asserted (§15.34 narrow side; the replace keeps the same
-- type signature so the existing grants survive regardless).
-- ----------------------------------------------------------------------------
...
values ('0112', '{0112_register_family_batch_classification.sql}', 'register_family_batch_classification')
