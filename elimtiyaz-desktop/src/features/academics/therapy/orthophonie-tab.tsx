 ...
 * `therapy_speech` charge to the ledger automatically. The "Ouvrir &
...
function CreateOrthoFollowUpModal({
  ...
  /**
   * Epic 6.5 — Ouvrir & Encaisser (orthophonie):
   *   1. Creates the follow-up (which appends a `therapy_speech` charge
   *      to the ledger via Epic 4.4).
  ...
  async function handleCreateAndCollect() {
    ...
    const ctx: PaymentNavigationContext = {
      ...
      lineItems: [{
        itemId: `ortho-${res.value.id}`,
        category: "therapy_speech",
        label: `Orthophonie — Forfait Semestre (${stu.firstName} ${stu.lastName})`,
      ...
      allowPartial: false,
      originRoute: "academics.orthophonie.create_and_collect",
    };
    setCollectCtx(ctx);
    onOpenChange(false);
