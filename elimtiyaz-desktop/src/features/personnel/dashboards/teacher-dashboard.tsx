...
export function TeacherDashboard() {
  ...
  async function handleAssignHomework(data: HomeworkFormData) {
    ...
    if (res.ok) {
      toast.showSuccess(
        "Devoir publié",
        "Les élèves et parents ont été notifiés.",
      );
  ...
  const kpis: readonly DashboardKpi[] = [
    ...
    { label: "Mes élèves", value: totalStudents, icon: Users },
    {
      ...
      icon: BookOpen,
      trend: myHomework.length > 0 ? "À corriger" : undefined,
    },
    {
      label: "Appel à faire",
      value: myClasses.length,
      icon: ClipboardCheck,
      trend: myClasses.length > 0 ? "À traiter" : undefined,
    },
  ];
  ...
  const homeworkFields: readonly AutoFormField[] = [
    ...
    },
    {
      ...
      required: true,
      wide: true,
      placeholder: "Ex. Devoir Chapitre 5",
    },
  ...
  const pickerClassId =
    overlay?.kind === "subject-picker" ? overlay.classId : "";
    ...
    <>
        ...
        <div className="rounded-lg border bg-card p-4">
          ...
          {me === null ? (
            <p className="text-sm text-muted-foreground py-6 text-center">
              ...
              Contactez l'administrateur pour activer votre espace pédagogique.
            </p>
          ) : myClasses.length === 0 ? (
            <p className="text-sm text-muted-foreground py-6 text-center">
              Aucune classe ne vous est affectée pour le moment. Vous pouvez
              être désigné comme enseignant principal depuis la page Classes.
            </p>
          ) : (
            <div className="grid gap-3 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3">
              {myClasses.map((c) => (
                <div
                  key={c.id}
                  className="rounded-lg border p-3 flex flex-col justify-between space-y-3"
                >
                  <div>
                    <p className="font-semibold text-sm">{c.name}</p>
                    <p className="text-xs text-muted-foreground">
                      Salle : {c.room ?? "—"} · {c.enrolledCount} élèves
                    </p>
                  </div>
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex gap-2">
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          setOverlay({ kind: "roll-call", classId: c.id })
                        }
                      >
                      ...
                      <Button
                        ...
                        variant="outline"
                        onClick={() =>
                          setOverlay({ kind: "subject-picker", classId: c.id })
                        }
                      >
      ...
      {(overlay?.kind === "roll-call" || overlay?.kind === "grades") && (
        <div
          ...
          aria-modal="true"
          aria-label={
            overlay.kind === "roll-call" ? "Appel" : "Saisie des notes"
          }
        >
