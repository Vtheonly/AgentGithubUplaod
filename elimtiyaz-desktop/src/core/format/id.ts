 ...
 *   Backup file   → backup-YYYY-MM-DD-HHMMSS[-ms].db
 */
...
/**
 * T-415 (BKUP-505): the millisecond suffix disambiguates two backups
 * created within the same second — previously both minted the SAME id and
 * the second storeArchive silently OVERWROTE the first archive. Milliseconds
 * suffice because a single runBackup (PBKDF2 + gzip + GCM) always spans
 * >1ms; the suffix is omitted only when ms === 0 to keep the legacy shape.
 */
export function backupFileName(now: Date = new Date()): string {
  ...
  return ms === 0 ? `${base}.db` : `${base}-${String(ms).padStart(3, "0")}.db`;
}
