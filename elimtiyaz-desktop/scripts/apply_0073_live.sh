#!/bin/bash
# T-179 / MIG-TOKENS pattern (AGENTS.md §15 rule 10): apply migration 0073 live
# with its schema_migrations registration in ONE atomic transaction.
# Usage: SUPABASE_ACCESS_TOKEN=... bash apply_0073_live.sh
set -euo pipefail
...
MIGRATION_FILE="$(dirname "$0")/../supabase/migrations/0073_suppliers_category_rating.sql"
...
PAYLOAD=$(mktemp /tmp/apply_0073.XXXXXX.sql)
{
  echo "BEGIN;"
  cat "$MIGRATION_FILE"
  echo "COMMIT;"
...
echo "Applying 0073 to ${PROJECT_REF} (atomic)…"
HTTP_CODE=$(curl -s -o /tmp/apply_0073_response.json -w "%{http_code}" \
...
echo "HTTP ${HTTP_CODE}"
head -c 600 /tmp/apply_0073_response.json 2>/dev/null || true
echo ""
rm -f "$PAYLOAD"
