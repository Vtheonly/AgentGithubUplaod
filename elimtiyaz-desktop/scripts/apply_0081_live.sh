#!/bin/bash
# T-223 / MIG-TOKENS pattern (AGENTS.md §15 rule 10): apply migration 0081
# live with its schema_migrations registration in ONE atomic transaction.
# Usage: SUPABASE_ACCESS_TOKEN=... bash apply_0081_live.sh
set -euo pipefail
...
MIGRATION_FILE="$(dirname "$0")/../supabase/migrations/0081_workflow_dag_execution_alignment.sql"
...
PAYLOAD=$(mktemp /tmp/apply_0081.XXXXXX.sql)
{
  echo "BEGIN;"
  cat "$MIGRATION_FILE"
  echo "COMMIT;"
...
echo "Applying 0081 to ${PROJECT_REF} (atomic)…"
HTTP_CODE=$(curl -s -o /tmp/apply_0081_response.json -w "%{http_code}" \
...
echo "HTTP ${HTTP_CODE}"
head -c 600 /tmp/apply_0081_response.json 2>/dev/null || true
echo ""
rm -f "$PAYLOAD"
