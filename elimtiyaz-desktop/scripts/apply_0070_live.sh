#!/bin/bash
# T-175 / MIG-TOKENS pattern (AGENTS.md §15 rule 10): apply migration 0070 live
# with its schema_migrations registration in ONE atomic transaction.
# Usage: SUPABASE_ACCESS_TOKEN=... bash apply_0070_live.sh
set -euo pipefail
...
MIGRATION_FILE="$(dirname "$0")/../supabase/migrations/0070_calendar_priority_assignment.sql"
...
PAYLOAD=$(mktemp /tmp/apply_0070.XXXXXX.sql)
{
  echo "BEGIN;"
  cat "$MIGRATION_FILE"
  echo "COMMIT;"
...
echo "Applying 0070 to ${PROJECT_REF} (atomic)…"
HTTP_CODE=$(curl -s -o /tmp/apply_0070_response.json -w "%{http_code}" \
...
echo "HTTP ${HTTP_CODE}"
head -c 600 /tmp/apply_0070_response.json 2>/dev/null || true
echo ""
rm -f "$PAYLOAD"
