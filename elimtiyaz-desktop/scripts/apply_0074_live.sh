#!/bin/bash
# T-180 / MIG-TOKENS pattern (AGENTS.md §15 rule 10): apply migration 0074 live
# with its schema_migrations registration in ONE atomic transaction.
# Usage: SUPABASE_ACCESS_TOKEN=... bash apply_0074_live.sh
set -euo pipefail
...
MIGRATION_FILE="$(dirname "$0")/../supabase/migrations/0074_tasks_display_names.sql"
...
PAYLOAD=$(mktemp /tmp/apply_0074.XXXXXX.sql)
{
  echo "BEGIN;"
  cat "$MIGRATION_FILE"
  echo "COMMIT;"
...
echo "Applying 0074 to ${PROJECT_REF} (atomic)…"
HTTP_CODE=$(curl -s -o /tmp/apply_0074_response.json -w "%{http_code}" \
...
echo "HTTP ${HTTP_CODE}"
head -c 600 /tmp/apply_0074_response.json 2>/dev/null || true
echo ""
rm -f "$PAYLOAD"
