#!/bin/bash
# T-178 / MIG-TOKENS pattern (AGENTS.md §15 rule 10): apply migration 0072 live
# with its schema_migrations registration in ONE atomic transaction.
# Usage: SUPABASE_ACCESS_TOKEN=... bash apply_0072_live.sh
set -euo pipefail
...
MIGRATION_FILE="$(dirname "$0")/../supabase/migrations/0072_leave_requests_request_kinds.sql"
...
PAYLOAD=$(mktemp /tmp/apply_0072.XXXXXX.sql)
{
  echo "BEGIN;"
  cat "$MIGRATION_FILE"
  echo "COMMIT;"
...
echo "Applying 0072 to ${PROJECT_REF} (atomic)…"
HTTP_CODE=$(curl -s -o /tmp/apply_0072_response.json -w "%{http_code}" \
...
echo "HTTP ${HTTP_CODE}"
head -c 600 /tmp/apply_0072_response.json 2>/dev/null || true
echo ""
rm -f "$PAYLOAD"
