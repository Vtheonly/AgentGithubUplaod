#!/bin/bash
# SCHED-103 (2026-09-22): apply migration 0113 (the class_subjects/classes
...
#
# Usage:
#   SUPABASE_ACCESS_TOKEN=... bash apply_0113_live.sh
set -euo pipefail
...
MIGRATION_FILE="$(dirname "$0")/../supabase/migrations/0113_class_subjects_teacher_fk.sql"
...
PAYLOAD=$(mktemp /tmp/apply_0113.XXXXXX.sql)
{
  echo "BEGIN;"
  cat "$MIGRATION_FILE"
  echo "INSERT INTO supabase_migrations.schema_migrations (version) VALUES ('0113');"
  echo "COMMIT;"
} > "$PAYLOAD"

echo "Applying 0113 to ${PROJECT_REF} (atomic)…"
HTTP_CODE=$(curl -s -o /tmp/apply_0113_response.json -w "%{http_code}" \
...
echo "HTTP ${HTTP_CODE}"
head -c 2000 /tmp/apply_0113_response.json 2>/dev/null || true
echo ""
rm -f "$PAYLOAD"

echo ""
echo "POST-CHECK (registration + constraints validated + zero orphans):"
run_sql "SELECT (SELECT count(*) FROM supabase_migrations.schema_migrations WHERE version = '0113') AS registered, (SELECT count(*) FROM pg_constraint WHERE conname IN ('class_subjects_teacher_id_fkey','classes_homeroom_teacher_id_fkey') AND convalidated) AS validated_fks, (SELECT count(*) FROM public.class_subjects WHERE teacher_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.personnel p WHERE p.id = class_subjects.teacher_id)) AS cs_orphans, (SELECT count(*) FROM public.classes WHERE homeroom_teacher_id IS NOT NULL AND NOT EXISTS (SELECT 1 FROM public.personnel p WHERE p.id = classes.homeroom_teacher_id)) AS classes_orphans;"
echo ""
