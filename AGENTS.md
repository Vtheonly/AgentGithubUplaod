...
|---|---|---|---|
| Desktop (this repo) | `cd elimtiyaz-desktop && npm run typecheck && npm run lint` | `npm test` (vitest; the ONE test root is `src/tests/` since T-419/ADR-029 — includes the cross-platform suites under `src/tests/cross-platform/`; the equivalence pipeline layers are being unified behind `npm test` by T-419) | Do not run `npm start` headlessly; it launches Electron |
