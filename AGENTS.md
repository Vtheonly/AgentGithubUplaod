...
|---|---|---|---|
| Desktop (this repo) | `cd elimtiyaz-desktop && npm run typecheck && npm run lint` | `npm test` — the UNIFIED runner (T-419/ADR-029, issue #22 §30): Layer 0 typecheck + Layer 1 the full vitest suite under the ONE root `src/tests/` + Layer 2 the financial-equivalence TS pipeline (generator → desktop runner → mirror runner → tier-4 comparison [reported while PARITY-005 is open] → the sanity/canonical gate) + Layer 3 the environment-gated census (real Kotlin / backend / live-E2E — reported with reasons, never silent). The summary compares every run against `scripts/test-baseline.json` (BASELINE-MATCHED vs DEVIATION). Fast loops: `npm run test:vitest` (Layer 1 only) or `npm test -- --layer=2 --skip-typecheck`; targeted suites: `npm test -- --suite=<filter>` | Do not run `npm start` headlessly; it launches Electron |
...
1. **Never** force-push, rewrite history, squash old commits, or delete branches — as amended by §15.59 (2026-09-27, T-418/ADR-028): the forensic unit is the COMMIT GRAPH. A branch ref whose tip is an ancestor of `main` (freshly re-verified: `merge-base --is-ancestor` AND `rev-list --count main..<branch>` = 0, run after a `--prune` fetch immediately before the deletion) MAY be deleted — its commits remain reachable from `main`. A ref carrying UNMERGED commits MUST NOT be deleted; merge it properly first. UNMERGED history is the forensic evidence; a merged ref is a label.
...
39. **A gated-RPC live sandbox MUST install the caller's JWT claims itself, must seed through the platform's TRIGGERS, and must census NOT-NULL/CHECK/FK rules from `information_schema`/`pg_catalog` BEFORE the first run — and the ONLY trustworthy EXECUTE-mode evidence is a `begin; … raise exception … rollback;` transaction (T-416 / PURGE-500..503, 2026-09-26: the purge-button live verification took EIGHT iterations, and every single failure was a real find the green local gates could not teach). The rules: (a) the Supabase Management-API SQL endpoint runs as `postgres`, which is NOT a superuser on hosted Supabase and carries NO `request.jwt.claims` — a `has_role('super_admin')`-gated RPC correctly FAILS CLOSED for it (`{ok:false, forbidden}`). To exercise the true UI-path authorization from a SQL sandbox, sign in via GoTrue and `set local request.jwt.claims = '<the verified JWT payload>'` inside the transaction — the exact GUC PostgREST installs per request (the admin's email then also lands in the audit entry's actor). (b) Seeding `auth.users` fires `handle_new_auth_user()`, which creates BOTH the `user_profiles` row AND a pending `account_approval_requests` row keyed by `auth_user_id` — never pre-seed the profile manually (unique-key collision); resolve the trigger-created row by `auth_user_id` subselects, and remember every AFTER-INSERT trigger on the seeded tables (audit, chat-notify, touch-*) fires inside your transaction and rolls back with it. (c) Before seeding ANY live table, census its NOT NULL no-default columns, CHECK enums, FK targets, and triggers in ONE pass — the file chain lies by omission (`students.date_of_birth` NOT NULL; `calendar_events.kind` has no 'other'; `payment_allocations.charge_id` → `ledger_entries.id`, NOT installments). (d) PL/pgSQL report CASE branches must not mix types (`v->>'k'` is text, `v->'k'` is jsonb — 42804), and a marker-exception report must be extracted with a regex that stops at the last `}` (PG appends a CONTEXT line that breaks JSON.parse). (e) THE BIG ONE: `jsonb_object_length(jsonb)` DOES NOT EXIST — 0120's audit note would have killed every real EXECUTE at the audit write while every dry-run gate stayed green (the audit is execute-mode-only); a dry-run-green destructive RPC is NOT execute-proven until the sandbox runs the REAL execute path. (f) An unregistered live migration (`0118`) under the same function name is reconciled by dropping EVERY `pg_proc` overload first, and `pg_proc`'s `::regprocedure::text` signature text has NO spaces after commas — test regexes must use `/,\s*/`.** The full convention: `scripts/t-416-purge-live-verification.mjs` + `docs/recovery/t-416-live-verification.md`.
...
### 55. **The GoTrue admin-createUser metadata-timing trap + the mutual-RLS recursion + the zero-row read-back — the three T-413 live discoveries (2026-09-26, 97th session)**

(a) **THE METADATA-TIMING TRAP.** GoTrue's admin createUser RETURNS `app_metadata` in its response, but the auth.users INSERT row does NOT carry it yet when `handle_new_auth_user()` (the AFTER-INSERT trigger) fires — live-proven: an invited probe user (app_metadata.created_by_admin=true) still got requested_role='parent' + phone=NULL from the trigger's SELF-SIGNUP branch while the stored row showed the metadata afterwards. Consequence: **the 0054/SEC-108 invite path's role capture has NEVER worked through the raw admin API; every signup lands as 'parent'.** The student path therefore begins at the ADMIN's approval-time reclassification (approve-signup-request EF, `assign_role` parent↔student, audited) — never at signup. Do not "fix" this by trusting client metadata (that was SEC-108's whole point).

(b) **THE MUTUAL-RLS RECURSION.** Adding a parents policy that subqueries students while a students policy already subqueries parents (0019's students_parent_sees_own) creates a policy cycle: `42P17 infinite recursion detected in policy for relation "students"` on EVERY parent/student read for non-staff roles — 500s across the portal, not just the new policy's target. The fix is the documented mutual-reference pattern (the 0067 profile_has_staff_role precedent): a read-only SECURITY DEFINER helper (`is_own_parent_via_student`) does the lookup outside the caller's RLS, and the policy calls the helper (with an EXECUTE grant to authenticated). A NEW mutual policy MUST be live-probed with both tables read under the affected role before commit (the t-413-apply.py T9 pattern).

(c) **THE ZERO-ROW READ-BACK.** A "successful" UPDATE (no error) can be a SILENT NO-OP when the applicable SELECT policies fold away every row (the RLS-500 SELECT-fold lesson extends to UPDATE row visibility). Live evidence: the self-attach test reported HTTP 200 while saving NOTHING. RULE: every self-service write asserts its effect by RE-READING the row through the same session (the read-back) — a zero-row read is an ERROR, never a success. The website form and the verify script both implement this; keep it that way. And when testing RLS behaviour through the Management API: `SET LOCAL` statements persist for the WHOLE transaction — `reset role` + `reset request.jwt.claims` after every impersonation block, or later DO blocks silently inherit the polluted context (the E2E9 cascade).


### 56. **A window/gap predicate must name its reference point EXPLICITLY — "current", "latest", and "as-of" are three different anchors, and a boundary test must exist for each (T-412 second round / WORKFORCE-505/506, 98th session, 2026-09-26)**

Both second-round payroll-forecast defects were the same bug class: a predicate silently keyed on the WRONG reference point. (a) The wave-set gate `personnel.some(p => isEligibleFor(p, currentPeriod))` answered "is anyone eligible for the CURRENT period?" while the code needed "would ANY projected period produce a wave?" — a future-hired roster (the natural pre-academic-year state) rendered zero waves and the feature's core purpose (pre-payroll funding visibility) disappeared exactly when it matters most. (b) The missed-payroll gap anchored on `recordedPeriods.at(-1)` — the max over ALL periods INCLUDING in-flight current-period rows — so recording the current payroll in progress silently erased every earlier missed month from the funding totals; the honest signal survived only in the recording pattern nobody used (no current rows yet). RULES: (i) when a predicate compares against a window boundary, NAME the anchor in the identifier (`lastRecordedBeforeCurrent`, not `lastRecordedPeriod`) and derive it with an explicit filter, never an unfiltered aggregate max; (ii) a "does anything qualify?" gate must be evaluated over the SAME domain the loop iterates (per-period), or it must not exist — an outer gate can only ever REMOVE cases the inner guard would have kept; (iii) every such boundary gets a dedicated test on BOTH sides of the edge (the inclusive hit AND the one-step-out miss — R2-2/R2-9/R2-13 are the models), plus one LIVE scenario when the write path is involved (the E2E STEP 5 pattern). The first round's 47 tests were all mid-window cases; the second round's 23 boundary tests found in one hour what a full green suite had hidden for a day.


### 57. **The T-415 hardening discoveries — PL/pgSQL OUT-parameter collisions, conflict-guard verdicts, the live-schema authority, and archive-id uniqueness (T-415 / BKUP-501..507 + SYNC-108/109/111, 99th session, 2026-09-26)**

(a) **A PL/pgSQL `RETURNS TABLE` clause declares OUT PARAMETERS that share the body's namespace with column references.** `returns table(archive_id uuid, file_name text, …)` + an unqualified `select id, file_name from public.backup_archives` inside the body = Postgres **42702 "column reference is ambiguous" on EVERY call** — the 0022 `purge_expired_backups` RPC shipped with exactly this and NEVER executed (the weekly EF logged "Purge failed" and continued, silently, since deployment; live-caught by the T-415 verification script). RULE: alias the table (`from public.t x`) and qualify EVERY column reference (`x.file_name`) in any SECURITY DEFINER body with RETURNS TABLE — and give a deployed RPC ONE live round-trip before calling it "working".

(b) **A conflict guard's `null` is not "push the local payload" — it is "I have no objection".** When the base, the local edit and the live remote row are compared, the auto-merged tree may carry the remote's NON-conflicting changes (fields the local edit never touched); pushing the bare local payload then SILENTLY REVERTS them (SYNC-108). The guard contract is a VERDICT: `{ conflict, mergedPayload }` — the drain pushes the merged payload when supplied. Corollaries: the BASE payload must ride through the same alias projection as the local (SYNC-111 — camelCase mock-store rows vs snake_case PostgREST rows otherwise never meet, producing phantom added-vs-added conflicts on every aliased field); and `resolveConflict` must ADVANCE the basePayload to the adjudicated remote state (SYNC-109 — a stale base makes the resolver re-detect the same conflict forever; only "remote" choices could ever terminate).

(c) **The LIVE schema is the authority — the migration files are history, not state.** Live discoveries: migration 0007's `receipts` table does NOT exist on the live project (only `pending_receipts`), and an unregistered live migration `0118 / purge_student_parent_domain` exists in `supabase_migrations.schema_migrations` with NO repo file. RULE: schema-dependent code and live scripts must INTROSPECT (`information_schema.tables`) instead of trusting the file chain's table inventory; before writing a new migration, read the LIVE registry (`select version from supabase_migrations.schema_migrations order by version desc`) — the file census alone can be stale by one or more numbers.

(d) **An archive id minted at second granularity is an overwrite bomb** (BKUP-505: two `backupFileName()` calls in the same second → the same id → the second storeArchive silently destroys the first archive). Millisecond suffixes fix it. And **`tryResult` must pass AppError-shaped throws through** (BKUP-506: `toAppError` re-wrapped every object as ERR_UNKNOWN, discarding the typed code + the actionable userMessage — the backup passphrase guidance never reached the operator). A caught object with `code: string` + `message: string` IS an AppError — return it unchanged.

### 58. **The T-417 discoveries — silently-skipping workbook suites rot their pinned anchors, cast-probed private seams are invisible to reachability greps, and every optional bulk method needs its mock implementation in the SAME change (T-417 / PERF-503, 101st session, 2026-09-27)**

(a) **A `describeOrSkip` workbook suite that cannot find its file contributes NOTHING while staying green** — the IMPORT-106 empty-state oracle and the T-105 shape suite had been silently SKIPPING on fresh clones for weeks (their `XLSX_CANDIDATES` listed only the pre-`Excel/`-reorganization locations), so their pinned census anchors (201 layout rows skipped) rotted invisibly behind the T-414 sheet-detection rework (now 202). RULE: a workbook-dependent suite's candidate list must include EVERY location the repo has ever kept the workbook (the real-excel-import suite's six-path list is the convention), and re-pinning a tripped anchor requires the stash-verified drift proof (the ORIGINAL code must produce the NEW number before the change under test can be blamed).

(b) **A test that reaches a private method through a type cast (`adapter as unknown as { findExistingStudent(...) }`) is INVISIBLE to the `rg "methodName"` reachability check** — removing the "dead" method breaks the suite at RUNTIME, not at grep time. RULE: before deleting any method, grep the cast shape too (`rg "as unknown as \{"` + the method name), and prefer probing PRIVATE seams through a PUBLIC contract (the t-364 IMPORT-109 probe now drives `upsertRecordsBatch` and asserts the write targets — a stronger pin than the private lookup ever was).

(c) **Every OPTIONAL bulk method on a repository interface (`bulkAppend?`/`bulkCollect?`/`bulkImportInstallments?`) hides a mock-parity landmine behind its `typeof === "function"` fallback guard** — IMPORT-108 was this for payments (2026-09-14); PERF-503 part (c) was the SAME defect for installments (the mock never got the method, so every mock-mode import slept 120 ms × 1,962 rows ≈ 4 minutes at the flush). RULE: when a repository gains an optional bulk method, the mock implementation lands in the SAME change — the fallback loop is a correctness hazard that manifests as pure latency, which no functional test ever catches.

(d) **The concurrency unit for a batched import is the FAMILY, not the row.** Parent-scoped identity resolution is the independence boundary: rows sharing a parent identity MUST be processed in row order within one task (parent create-once, sibling resolution, duplicate-row update chains, per-row retry-after-failure), while DIFFERENT families may run concurrently. The cross-family duplicate-create race (two sibling rows both missing the same not-yet-created parent) is killed by a SEQUENTIAL in-memory pre-resolution pass (Phase 1) that groups rows into families BEFORE any write starts — never by locking.

(e) **The repository round-trip COUNT is the import's performance invariant, not the wall clock** (the PERF-501/502 convention, now pinned by a suite): the perf suite asserts `parents.search ≤ 3 && students.search ≤ 3` (the snapshot reads) while pinning `createParent === 253 && createStudent === 390 && updateStudent === 0` EXACTLY — the canonical writes must NEVER shrink to make the budget. An optimization that reduces the write count is a BEHAVIOR change (skipped processing), not a speedup.

### 59. **The T-418 discoveries — the branch namespace is residue, not inventory; §15.1's forensic unit is the commit graph; and a "merge the branches" mandate begins with a containment census, not a merge (T-418 / OPS-322 / ADR-028, 102nd session, 2026-09-27)**

(a) **A "merge or remove all branches safely" mandate (issue #21) begins with the CONTAINMENT CENSUS, not with merges.** The forensic census (per branch: `git merge-base --is-ancestor origin/<b> origin/main` + `git rev-list --count origin/main..origin/<b>`) found every one of the 39 non-main refs (36 hub + 3 website) was ALREADY an ancestor of main with ZERO unique commits — the merge-after-each-commit discipline of sessions 58–101 had already done the merging; the branch namespace was pure label residue. When an owner asks to "merge branches safely", ALWAYS run the census first: if everything is already contained, the risk surface moves from conflict resolution to (1) per-branch containment proof, (2) health gates on the combined main BEFORE any irreversible step, and (3) preserving the name→tip→merge-commit map in the docs tree before the labels disappear. The name→commit map is the ONLY thing a branch deletion destroys (ADR-028).

(b) **§15.1's literal text collided with the owner's explicit mandate — and the resolution is the commit-graph rule.** "Never delete branches. Git history is forensic evidence" was written to protect UNMERGED work, but read literally it also froze 39 merged labels forever. ADR-028 resolves it: the forensic unit is the COMMIT GRAPH; a ref whose tip is an ancestor of main is deletable (fresh re-verification immediately before the deletion — a concurrent agent may have pushed to it since your census); a ref with unmerged commits must be merged properly first. The deletion itself provably touches nothing: `git rev-parse origin/main` is byte-identical before/after, the working tree is untouched, `git status` stays clean — which is why pre-deletion gates remain valid post-deletion (the invariance proof, not a re-run of the suite).

(c) **The branch list is NOT a work inventory — `docs/recovery/task-registry.md` is.** 37 finished-task labels masquerading as an open-work list cost every future session an enumeration+dismissal round at open. The single-active-branch model (ADR-028 rule 4): task branches live only for their session (push → merge → delete ref, all in-session); the branch namespace stays at `main` + the current session's live branches. An agent seeing a branch list longer than that should treat it as cleanup residue, not as someone's in-flight work — but MUST still run the census before deleting (a branch genuinely mid-flight from a concurrent agent will show unique commits and is NOT residue).

(d) **GitHub push protection is an ACTIVE push-blocking enforcement layer on this repository — the §15.12 secret rule is machine-enforced.** The session's first Phase-0 push was rejected wholesale (`GH013 … Push cannot contain secrets — Supabase Personal Access Token, path: scripts/t418-live-smoke.sh:5`) because the smoke script carried the owner-supplied `sbp_` token inline in a "read-only" script. The amend-allowed case (§5: local-only + unpushed + same-session) applied; the script now reads `SUPABASE_ACCESS_TOKEN`/`SUPABASE_ANON_KEY` from the environment. Rule: ANY script that touches live infrastructure takes credentials via env vars at the boundary — an inline literal blocks the entire push, even in tooling, even read-only.
