...
**Boundaries:** the purge is tenant-wide (the issue's "full reset" — not per-student); `account_approval_requests` go only when `target_parent_id`/`target_student_id` links them to purged rows (an unlinked pending signup is pre-parent data); the auth closure skips nothing but deletes `auth.users` only for accounts whose profile held parent/student portal roles (a staff account is never caught in the blast radius — the role-assignment guard).
