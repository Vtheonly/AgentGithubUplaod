...
- **Amended (2026-09-26, the 98th session's second-round verification — WORKFORCE-505/506, see financial-rules §16):** clause 4's eligibility is evaluated PER PERIOD (no current-period gate on the wave set — a future-hired roster still gets its future waves), and the missed-payroll gap anchor is the last recorded period STRICTLY BEFORE the current period (current-period rows can never mask earlier gaps). Interior gaps remain a documented owner decision (UNKNOWN-025).
