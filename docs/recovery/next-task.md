...
**13th repair session (2026-08-31, in progress)** — owner-requested batch of ~10 tasks. DONE so far: **T-041** (migration 0059 live 10/10), **T-030** (migration 0060 live 9/9), **T-058** (append-only migration guard), **T-050** (OnlineDetector, both platforms), **T-036 PUSH-103 portion** (website FCM auto-registration). DONE: **T-050** (OnlineDetector fail-closed + own-backend probe + pullAll dedup — Android commit 3bc5cdd, desktop in the hub; Android debug suite 234/234, desktop 2165; NEW finding ARCH-012 registered) and **T-036 PUSH-103 portion** (website FCM auto-registration after first user gesture — 9/9 new tests, suite 144/144, strict build green). REMAINING: **the Android set** (unblocked PUSH-103 portion: website FCM auto-registration after first user gesture), and the Android set **T-026, T-054, T-062, T-063, T-064** (dl.google.com reachable again — toolchain re-bootstrapped at /home/z/my-project/tools; gradle memory-tuned for the 2-CPU container). Session opening completed the owner's two standing asks: portal `.env.local` (real Supabase URL + anon key — the "Missing configuration" banner is gone, live headless render verified) and the live migration-token consistency check (57/57 rows = 0001–0060). NEW discovery registered: **AUTH-200** — Google OAuth is NOT enabled on the live project (provider disabled, no client id/secret) — the portal's ONLY auth path is dead until the owner configures it (runbook in docs/operations/portal-google-oauth.md).

## Current recommendation

**The Android set — T-026, T-054, T-062, T-063, T-064** (the remaining 13th-session batch; toolchain is READY at /home/z/my-project/tools). Order by risk/value: **T-062** first if taking the P0s in registry order — check each entry's actual priority in task-registry.md before starting. Environment recipe: `export JAVA_HOME=/home/z/my-project/tools/jdk-17`; `./gradlew testDebugUnitTest --offline` (the full `test` task carries the pre-existing release-variant failure ARCH-012 — debug is the green gate); kill stale Gradle/Kotlin daemons first if the daemon disappears (2-CPU/4GB container). After the Android set: session closeout (worklog, zip for push).
