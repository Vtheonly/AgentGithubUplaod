...
**Nothing in progress — the 15th repair session (2026-09-01) is CLOSED: T-103 COMPLETE (the owner-reported Finance-tab vs parent-dossier divergence is fixed end-to-end — migration 0062 financial reconciliation applied live + registered [chain 59/59 = 0001–0062, zero drift] + the desktop read paths made canonical; live verification 8/8 with 0/258 residual mismatches; desktop 68 files / 2187 tests ALL PASS; the owner's exact parent spot-checked).** DATA-001/002/003/004 flipped VERIFIED; DATA-008 registered+VERIFIED (the owner report); DATA-009 registered (canonical parent_credit double-count → T-104). Full evidence: docs/recovery/t-103-live-verification.md.

## Current recommendation

**Next session should pick from the Ready set, in this order:** **T-104** (NEW — DATA-009 parent_credit double-count decision: write an ADR choosing between fixing the canonical writer's payment-entry amount vs a display-level convention deriving credit from `totalUnallocatedCredit`; the equivalence suites pin the current shape, so this is a design decision, not a patch), then **T-024** (Android homework id + promotion propagation — Critical, unblocked, needs the Android toolchain re-bootstrapped), **T-017** (Android refunds idempotent), **T-020/T-021** (Android sync semantics), **T-102 follow-up** (build the Android chat screen — read-side + online sends feasible now), then **T-034** (student transfer flow). The financial data is now consistent across all platforms; the remaining finance work is the DATA-009 design decision + DATA-005 (first_name split — cosmetic). TEST-300 lesson for every session: run the FULL suite on a pristine tree before closeout and record per-file evidence.
