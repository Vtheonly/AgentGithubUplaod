...
*(none — the seventh repair session (2026-08-30) closed T-079 (live round-trip → VERIFIED) and T-004 (live curl matrix → VERIFIED); five more tasks TESTED: T-016, T-027, T-061, T-031, T-029, T-071. Evidence in change-log.md.)*

**T-083 follow-up note:** during T-004's live curl matrix, a NEW bug was discovered: `expire_pending_approvals()` SQL RPC references a non-existent `public.users` table — the daily cron has been silently failing. The auth gate (T-004 / SEC-105) is verified; the SQL RPC bug is a separate task (T-083).

## Current recommendation

**T-005 — tenant-scoped RBAC resolver + admin policies** (TENANT-100/101, P0 Critical, no dependencies): new migration 0049+ (0048 is the latest applied); SQL-level behaviour fully specified in the problem entries; implementation + migration review are headless-feasible, with live two-tenant tests as the recorded gap (same pattern as T-004 — now achievable since the live environment is wired up). Alternative client-side pick: **T-082** (Android lint-gate baseline — the last inoperable AGENTS.md §6 gate, same pattern as T-078). **T-083** (fix `expire_pending_approvals` SQL RPC — discovered during T-004 verification) is also a strong next-task candidate: small backend migration, can reuse the live-curl-matrix verification pattern from T-004.
