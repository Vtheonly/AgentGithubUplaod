...
**13th repair session (2026-08-31, in progress)** — owner-requested batch of ~10 tasks. DONE so far: **T-041** (year-end promotion — migration 0059 live 10/10), **T-030** (FCM lifecycle — migration 0060 live 9/9), **T-058** (append-only migration guard — machine-enforced), **T-050** (OnlineDetector fail-closed + own-backend probe + pullAll dedup — both platforms). DONE: **T-050** (OnlineDetector fail-closed + own-backend probe + pullAll dedup — Android commit 3bc5cdd, desktop in the hub; Android debug suite 234/234, desktop 2165; NEW finding ARCH-012 registered: release-variant GreetingScreenshotTest pre-existing failure). REMAINING: **T-036** (unblocked PUSH-103 portion: website FCM auto-registration after first user gesture), and the Android set **T-026, T-054, T-062, T-063, T-064** (dl.google.com reachable again — toolchain re-bootstrapped at /home/z/my-project/tools; gradle memory-tuned for the 2-CPU container). Session opening completed the owner's two standing asks: portal `.env.local` (real Supabase URL + anon key — the "Missing configuration" banner is gone, live headless render verified) and the live migration-token consistency check (57/57 rows = 0001–0060). NEW discovery registered: **AUTH-200** — Google OAuth is NOT enabled on the live project (provider disabled, no client id/secret) — the portal's ONLY auth path is dead until the owner configures it (runbook in docs/operations/portal-google-oauth.md).

## Current recommendation

**T-036 — Rebuild the push notification pipeline (PUSH-103 portion only — the unblocked part of the task)**: website FCM auto-registration after the first user gesture. Browsers require a user gesture for `Notification.requestPermission()` — the pattern: on the FIRST meaningful gesture after sign-in (any click/keypress), attempt permission + registration ONCE (per browser session, gated by a localStorage flag), keep the Profile toggle as the explicit control. The T-030 groundwork is already in place (fcm-registration.ts: registerDeviceToken persists the last-known token; rotation retire wired; the register RPC is guarded by migration 0060). Files: `src/lib/hooks/fcm-registration.ts` + `src/app/providers/auth-provider.tsx` + a new test file. SKIP the still-blocked T-036 parts (PUSH-100 EF invocation path, PUSH-104 email provider — owner decisions pending). After T-036: the Android set **T-026, T-054, T-062, T-063, T-064** (toolchain ready; use `./gradlew testDebugUnitTest --offline` with JAVA_HOME=/home/z/my-project/tools/jdk-17; the full `test` task has the pre-existing release-variant failure ARCH-012 — debug is the green gate).
