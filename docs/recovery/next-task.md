...
*(none — the eighth repair session (2026-08-30, owner-requested) closed T-083 + T-084: live backend health check (11 findings → DATA-001…007, BUG-NEW-002/003), migrations 0049+0050 applied LIVE and verified, website portal UI restructured around the real data model, FCM token lifecycle hardened cross-platform (SEC-106 TESTED / SYNC-105 TESTED / SYNC-104 IMPLEMENTED), credentials sheet written. Evidence in change-log.md.)*

**T-083 closed note:** the `expire_pending_approvals` `public.users` bug discovered during T-004 was fixed by migration 0049 (applied live, verified — correct table + clean execution). Remaining: EF round-trip with CRON_SECRET.

## Current recommendation

**T-085 — live-data reconciliation** needs the OWNER first (not a code task): decisions on parent e3e90f1f's 1,750/10,000 DZD discrepancies, the 7.62M of ledger-only charges, and whether to replay the 888 payments through the canonical waterfall. Present `docs/audits/backend-health-check-2026-08-30.md` to the owner.

For the next CODE session: **T-005 — tenant-scoped RBAC resolver + admin policies** (TENANT-100/101, P0 Critical, no dependencies). The live deployment path is now proven in-session (Management API SQL endpoint — see the credentials sheet's verification checklist), so 0051+ can be written, applied live, and SQL-verified the same day; live two-tenant tests are the recorded gap. Alternative picks: T-082 (Android lint baseline), T-087 (test-residue cleanup — small, live-applyable).

Session outcome (2026-08-30, eighth session — owner-requested): live backend health check executed against production (98 tables, 34-table row census, three-way per-parent financial reconciliation, RLS probes clean, 58-RPC inventory, auth census) → 11 findings registered (DATA-001…007, BUG-NEW-002/003); migration 0049 applied live (KPI fan-out 21.38B→54.96M verified byte-identical, MV unique indexes + CONCURRENTLY refresh verified, expire_pending_approvals fixed); migration 0050 applied live (register_fcm_token caller verification + deactivate_fcm_tokens RPC); website portal restructured around the real data model (NEW ledger statement timeline with running balance, adjustments from the ledger, real payment statuses, display_name greeting, financial-first dashboard, fr/ar/en i18n, honest empty states) — strict build green, lint CLEAN for the first time, 105/105 tests; SYNC-105 fixed (local scope + token unregister); SYNC-104 implemented (Android sign-out deactivation; compile check pending); docs/operations/credentials.md written. T-083 + T-084 TESTED.
