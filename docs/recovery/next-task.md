...
**Nothing in progress — the 13th repair session (2026-08-31) is CLOSED: 10/10 tasks of the owner-requested batch complete.** T-041 (migration 0059 live 10/10), T-030 (migration 0060 live 9/9), T-058 (append-only migration guard), T-050 (OnlineDetector, both platforms), T-036 PUSH-103 portion (website FCM auto-registration), and the Android set **T-026, T-054, T-062, T-063, T-064** (Android suite 275/275, +41 tests; debug APK 29.8 MB). Session standing asks completed: portal `.env.local` ("Missing configuration" banner gone, live render verified), live migration-token check (57/57 = 0001–0060 at open). AUTH-200 (Google OAuth provider not enabled live) remains owner-actioned — runbook docs/operations/portal-google-oauth.md.

## Current recommendation

**Session closeout is done — next session should pick from the Ready set.** With the 13th-session batch complete, the recommended next batch (in registry priority order): **T-017** (website EF consolidation leftovers), **T-024** (Android promotion propagation — now unblocked: T-041's canonical RPCs exist), **T-034** (student transfer flow), then the owner-decision gates (T-085/T-086) if the owner has answered the unknowns. Environment recipe for Android work: `source /home/z/my-project/tools/android-env.sh`; `./gradlew testDebugUnitTest --offline` (the full `test` task carries the pre-existing release-variant failure ARCH-012 — debug is the green gate); kill stale Gradle/Kotlin daemons first if the daemon disappears (2-CPU/4GB container).
