...
**13th repair session (2026-08-31, in progress)** — owner-requested batch of ~10 tasks. DONE so far: **T-041** (year-end promotion — migration 0059 live 10/10, desktop 2146→2152 green), **T-030** (FCM lifecycle — migration 0060 live 9/9, website 135/135; both halves committed), **T-058** (append-only migration guard — machine-enforced via npm test + check:migrations; matrix 9/9, suite 6/6; 3 pre-existing T-041 TS errors fixed en route). REMAINING: **T-050** (OnlineDetector fail-closed + Supabase probe target, both platforms — NEXT), **T-036** (unblocked PUSH-103 portion: website FCM auto-registration after first user gesture), and the Android set **T-026, T-054, T-062, T-063, T-064** (dl.google.com reachable again — toolchain re-bootstrapped at /home/z/my-project/tools; gradle memory-tuned for the 2-CPU container). Session opening completed the owner's two standing asks: portal `.env.local` (real Supabase URL + anon key — the "Missing configuration" banner is gone, live headless render verified) and the live migration-token consistency check (57/57 rows = 0001–0060). NEW discovery registered: **AUTH-200** — Google OAuth is NOT enabled on the live project (provider disabled, no client id/secret) — the portal's ONLY auth path is dead until the owner configures it (runbook in docs/operations/portal-google-oauth.md).

## Current recommendation

**T-050 — OnlineDetector fail-closed + Supabase probe target** (SYNC-108-family; P2, both platforms): the online/offline gate decides whether the Android queue replays writes — a flaky/optimistic detector under a captive portal silently strands mutations (T-020/T-021 depend on it). Plan: find the problem entry's exact expectations in problem-registry.md (probe the REAL backend health endpoint instead of a generic URL; fail CLOSED on ambiguous results), implement on both Android (ConnectivityManager-based detector) and desktop/website equivalents where present, unit tests for the fail-closed matrix. After T-050: **T-036** (website FCM auto-registration after first user gesture), then the Android set **T-026, T-054, T-062, T-063, T-064** (toolchain ready at /home/z/my-project/tools; run `./gradlew test` with the memory-tuned gradle.properties).
