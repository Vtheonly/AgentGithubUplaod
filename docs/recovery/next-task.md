...
**13th repair session (2026-08-31, in progress)** — owner-requested batch of ~10 tasks. DONE so far: **T-041** (year-end promotion — migration 0059 live 10/10, desktop 2146 green), **T-030** (FCM lifecycle — migration 0060 live 9/9, website 135/135; both halves committed). REMAINING: **T-058** (append-only migration discipline guard — NEXT), **T-050** (OnlineDetector fail-closed + Supabase probe target, both platforms), **T-036** (unblocked PUSH-103 portion: website FCM auto-registration after first user gesture), and the Android set **T-026, T-054, T-062, T-063, T-064** (dl.google.com reachable again — toolchain re-bootstrapped at /home/z/my-project/tools; gradle memory-tuned for the 2-CPU container). Session opening completed the owner's two standing asks: portal `.env.local` (real Supabase URL + anon key — the "Missing configuration" banner is gone, live headless render verified) and the live migration-token consistency check (57/57 rows = 0001–0060). NEW discovery registered: **AUTH-200** — Google OAuth is NOT enabled on the live project (provider disabled, no client id/secret) — the portal's ONLY auth path is dead until the owner configures it (runbook in docs/operations/portal-google-oauth.md).

## Current recommendation

**T-058 — Adopt append-only migration discipline** (REG-001; P2 High, process): the chain just grew to 60 files (0059 + 0060 this session) — the guard is due NOW, before more migrations land. Plan: a committed check script (`scripts/check-migrations-append-only.sh`) that fails when a diff vs the merge-base shows modifications/deletions under `supabase/migrations/` (additions only), plus a header-discipline lint for new migrations; wire it into the desktop test pipeline so `npm test` enforces it; verify it catches a planted edit (fail case) and passes clean (pass case). Small, self-contained, unblocked. After T-058: **T-050** (OnlineDetector fail-closed + Supabase probe target), then **T-036** (website FCM auto-registration), then the Android set **T-026, T-054, T-062, T-063, T-064**.
