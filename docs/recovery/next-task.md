...
*(none — the ninth repair session (2026-08-30, owner-requested) closed T-088 + T-080 + T-089 + T-091 + T-087 + T-092: desktop Dashboard UI restructured (ARCH-010 — 5 duplication/dead-code defects fixed); 4 hardcoded Supabase KPIs backfilled against real data (totalStaff / pendingExpenses / attendanceRateToday / overdueAlerts — verified live); SupabaseOverdueAlertGenerator ported (ARCH-006 closed — 8-test unit suite); migration 0050 drift reconciled (ARCH-009 — 0051_chat_read_receipts.sql added + applied live + registered in schema_migrations); test-residue cleanup (DATA-007 closed — 0052_drop_test_residue.sql applied live, test auth user + expired approval request deleted); cross-platform migration token consistency verified (7/7 checks pass). Evidence in change-log.md.)*

## Current recommendation

**T-093 — Port desktop `expenses` repository to Supabase (DRIFT-013, P2 High)** is the natural follow-up: T-089 mitigated the dashboard KPI half (queries `expense_tickets` with `status='pending_approval'` directly), but the wider expenses-repository leak — the assembly still uses `MockExpensesRepository` and the desktop domain enum (`submitted|approved|...`) doesn't match the DB column values (`pending_approval|approved_funds_released|...`) — is still open. Plan: implement `SupabaseExpenseRepository` with a translation layer, override the `expenses` slot, decide whether to align the desktop enum to the DB values (preferred per AGENTS.md §15.9 + ADR-001) or keep a mapping layer.

**T-094 — Live integration test for `SupabaseOverdueAlertGenerator` (T-080 follow-up, P2 Medium)** is the other natural follow-up: T-080 closed the mock-leak defect with 8 unit tests (fake Supabase client). The live-integration verification — run the generator against the real Supabase backend, verify new notifications insert (or the dedup path returns 0 new if all installments already have alerts), verify the audit entry appears — is this task. The live deployment path is now proven in-session (Management API SQL endpoint + dollar-quoting schema_migrations registration pattern).

Alternative pick: **T-005 — tenant-scoped RBAC resolver + admin policies** (TENANT-100/101, P0 Critical, no dependencies). Same pattern as T-091 (headless-implementable + live-verification via the Management API SQL endpoint).
