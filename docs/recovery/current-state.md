...
- **2026-08-31 — Thirteenth repair session (IN PROGRESS; owner-requested ~10-task batch):** portal `.env.local` configured (the "Missing configuration" banner is GONE — live headless render verified); mandatory live chain check 57/57 (0001–0060); **T-041 TESTED** (migration 0059 live 10/10); **T-030 TESTED** (migration 0060 live 9/9; website 14 files / 135 tests); **T-058 TESTED** (append-only migration guard machine-enforced — matrix 9/9, suite 6/6; 3 pre-existing T-041 TS errors fixed); **T-050 TESTED** (OnlineDetector fail-closed + own-backend probe + pullAll dedup, both platforms — Android commit 3bc5cdd, debug 234/234; desktop 65 files / 2165 tests); NEW problems registered: AUTH-200 (Google OAuth provider disabled — owner action, runbook in docs/operations/portal-google-oauth.md) and ARCH-012 (release-variant GreetingScreenshotTest pre-existing failure). Remaining batch: T-036 (PUSH-103 portion), T-026, T-054, T-062, T-063, T-064.
