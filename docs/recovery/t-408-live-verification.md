...
## 11. The 91st session — the FAKE-marked test dataset + the remaining live E2E (80/80 GREEN)

The owner authorized clearly-marked FAKE test data in the live project to run the remaining T-408 tests and to test the timetable functionality by hand afterwards. The full evidence is in `docs/recovery/t-408-academic-setup-creation.md` (the 91st-session section); the essentials:

- **`scripts/t-408-fake-academic-e2e.ts` — 80/80 GREEN** (the recorded final run): canonical-repository seeding (8 personnel → teacher flips → 2 FAKE subjects → 5 classes with REAL level/year uuids and homeroom FKs → 52 class_subjects / 118 weekly hours → 9 rooms → 3 constraints) → generation 118/118 placed, 0 hard violations, zero teacher/room/class double-bookings, the hard teacher-unavailability honored, required room types honored → submit → approve → **publish (the first LIVE `fn_timetable_publish` since 0109)** → `v_timetable_published` 118 rows admin / 401 anon → the interaction suite (Matières-Classes data per class, the teacher registry 8/8 active, the homeroom read-back, the T-404 manual-adjustment contract: duplicate → live-validated move → lock → regenerate → pin survives, still fully placeable).
- **`scripts/t-408-fake-data-purge.py` — verified 5× live** (three mid-development partial states + two full datasets): dry-run by default, `--execute` flips published FAKE versions to draft first (SCHED-109's trigger semantics), then FK-safe deletes with row-count asserts and post-checks — every run ended `PURGE COMPLETE — zero FAKE residue`, with the catalog (14 subjects / 127 configurations / 14 levels / the year), the owner's room `3`/"eee", and the owner's `Essai 1` in_review version asserted PRESERVED.
- **The dataset is INTENTIONALLY LIVE** for the owner's UI testing (the Emploi du temps tab, the Enseignants registry, the Matières-Classes assignments, the portal timetable). The FAKE-marker convention: AGENTS.md §15.50.
- **Five new findings registered** from the testing itself: SCHED-107 (room-fit vs nominal class capacity — labs at 24 seats rejected every 28+ class), SCHED-110 (the greedy's no-backtracking corner case at ≥90% class occupancy), SCHED-108 (moveEntry blocked on ANY unmet weekly hour), SCHED-109 (no unpublish path; published-entry immutability extends to cleanup — triggers fire through the service role), ACAD-510 (`classes.notes` silently dropped — mock-era model field, no column, never sent).
- **Gates:** tsc 0 · eslint 0 errors · FULL vitest 3749/21/5 = the byte-identical baseline (scripts/docs only — zero product code changed).

**T-408's teacher-creation VERIFIED gate: CLOSED** (live rows through the canonical paths). The remaining gate: the Android equivalence run.
