...
## UNKNOWN-026 — The cold-cache backup boundary: should a Supabase-mode runBackup WAIT for the repository caches to seed? (T-415, 2026-09-26, 99th session)

The Supabase repositories' reactive caches seed asynchronously on first `observe()` (fire-and-forget). A backup taken on a cold app — before any screen subscribed — snapshots empty collections. The archive's metadata counts honestly expose that (the Settings list + inspectArchive show 0 — the detection surface, pinned by the metadata-fidelity tests), but the honest question for the owner: should runBackup (a) pre-warm and AWAIT the seeds (needs a `whenSeeded()` surface on the shared repositories — 8 classes), (b) snapshot a fresh server read instead of the caches (a bigger re-architecture of snapshotState), or (c) keep the current documented boundary (backups are of the app's current data view; take one after the app loaded)? Until decided, the boundary is documented in the SupabaseBackupRepository header + the T-415 registry entry.

## UNKNOWN-027 — The Supabase-mode restore semantics: restore rehydrates the OFFLINE operating layer (T-300's design) — should a full server rehydration path exist for Supabase mode? (T-415, 2026-09-26, 99th session)

Per T-300, `restore()` applies the snapshot into the offline operating mode (the mock layer) and arms staging — post-restore mutations flow to the server through the sync queue. In Supabase mode the app's screens read the Supabase repositories' caches, so a restored snapshot is not what the UI shows until the data flows back through sync. The full server-side rehydration path EXISTS as the Excel-import restore (t-364/t-366, IMPORT-106 — the atomic empty-DB restore). The owner decision: (a) keep the two paths separate (local/offline restore for disaster recovery + the Excel path for server rebuilds — the current state, documented), or (b) unify (a Supabase-mode restore re-imports the whole snapshot through the canonical RPCs — a significant new surface with idempotency requirements the Excel path already solved).

## UNKNOWN-028 — The unregistered live migration 0118 + the live-schema divergence: who owns reconciling the repo's migration chain with the live database? (T-415 live discoveries, 2026-09-26, 99th session)

The live `supabase_migrations.schema_migrations` carries **0118 / purge_student_parent_domain** with NO corresponding file in the repository (the §15.14 unregistered-patch class — applied by an off-repo actor), and **migration 0007's `receipts` table is ABSENT live** (only `pending_receipts` exists) — the live database was not built by replaying the file chain verbatim. Per §57c the live schema is the authority (introspect; read the live registry for the next free number — it is **0120** after this session's 0119). The owner decision: audit + document 0118's content and the receipts divergence (a backfill file? a live-schema census doc?), or accept the divergence permanently with the introspection rule as the guard.
