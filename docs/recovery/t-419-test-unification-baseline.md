...
**Canonical expected 317/318:** the one unverified expectation is on the same all-error scenario (the error result cannot satisfy the numeric `then` block) — same root cause as TEST-308. **Deeper audit (Phase 3):** scenario 017's `then` block was itself STALE — it expected a no-op success (`totalOutstanding: 10000000`, `_comment`: "returns early with no allocations") while its own `description`, the desktop entry factory (`src/domain/calc/ledger/entries.ts` `require input.amount > 0`), and the SQL RPC (0034: "Validate amount > 0 (matches canonical require(amount > 0))") all say REJECTION. Corrected in Phase 3 to the error-expectation form (`then.error`).

### 2.4b The Tier-4 mirror surface — the first-ever completed tier4_comparator run (Phase 3 audit finding, registered PARITY-005)

Running the mirror runner + `tier4_comparator.ts` (never run in recorded history — one junk-era commit, zero documented runs, and its report writer CRASHED on `JSON.stringify(undefined).slice(...)` before producing output):

```
Android mirror runner: 784 passed, 0 failed, 0 errored, 35 skipped (of 819)
Tier-4 comparison:     707 equivalent / 35 skipped / 77 scenarios with divergences
                       499 discrepancy rows (112 ERROR, 387 WARNING)
```

- **488 of 499 rows are `evaluateAllSystemDiscounts` divergences** — the mirror still applies the CALC-001-REMOVED fictional discount rules (`passage_palier` ×94, `sibling_fixed` divergent ×76, `seniority_5y` ×62, `highest_average` ×34, `full_annual` ×12) while the canonical desktop engine returns none (scenario 007's canonical `then: {totalDiscount: 0}` confirms the corpus already encodes the post-CALC-001 expectations).
- **10 rows are the zero-payment family as error-vs-success** — desktop AND SQL reject `amount ≤ 0`; the mirror ACCEPTS (no validation in its entry factory).
- **1 reconcile representational row** (`violations[0].details` null on the mirror side).
- The **35 skipped** are ops the TS mirror runner does not implement (the app-layer/analytics/academic families) — the REAL Kotlin runner (`AndroidEquivalenceRunner.kt`) implements them; the TS runner's coverage gap is part of the registered follow-up. Reported with reasons, never silent (issue §20).

**Classification per issue #22 §7:** this is NOT cross-platform agreement to preserve — the two implementations AGREE on nothing here; the DESKTOP side is canonical (post-CALC-001 + the SQL RPC's identical validation), the MIRROR is the stale side. PARITY-005 owns the repair (mirror engine + the suspected real `DiscountEngine.kt`).
