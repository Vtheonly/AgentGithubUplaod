...
- **Mandate:** owner (2026-09-10) — "I integrated AI into the system, but the implementation is far too shallow and feels more like a beginner-level demo than a serious, production-ready integration... full of mistakes, inconsistencies, and incomplete functionality... I want a proper, professional AI integration that solves real problems, understands the system's existing data and workflows, and provides genuinely useful capabilities... not a shallow proof of concept." + "apply the migration tokens. Ensure that everything works correctly across all platforms and that the migration is properly applied and remains consistent everywhere."
- **Session-opening detection (§15.14 fired):** HEAD 6ce49b9 ("chore: refresh smartexport selection-state file registry", an author distinct from every session) actually rewrote 14 files of the AI layer (+1196/−881): deleted edgeLLMAdapter (the production server-key path), deleted the readRawStored seam, broke 5 tests + 4 type errors, stripped ~500 lines of docs — while also carrying a SOUND but unregistered SuperAdmin AI-gate fix. Attributed BEFORE any work was built on it; triaged (good parts registered as AI-309, damage repaired, everything else deepened).
- **What changed (hub repo — desktop + ai-proxy EF):**
  - T-266 (a461227): edgeLLMAdapter + edge→byok→mock routing RESTORED with full SEC-002 docs; readRawStored seam restored; mock ai-config-repository gained the fastModel/reasoningModel/enableSmartRouting merge fields; defaults pinned to REAL models; the patch's tool slicing REMOVED (a capability bug — "qui me doit de l'argent ?" lost every financial tool) while its model-tier routing KEPT for model selection only; Bearer-null fixed (keyless providers skipped); the fake console.warn guardrail removed.
  - T-267 (c7107a7): the tool registry 6 → 12 — get_overdue_accounts (debt.observeSummary: debtors, aging, phones, filters), get_collection_analytics (kpis + debtByAging: aging composition + collection rate), get_payment_history (receipts/methods/statuses), get_student_attendance (canonical calculateAttendanceRate + status breakdown + drop-off risk + justification status), get_class_performance (per-student evaluateStudentTermPerformance: class average, top/bottom, at-risk), get_student_academic_profile enriched with the 30d attendance rate; REAL proposal validation (parent exists, non-zero ≤ 5 000 000 DZD, reason ≥ 3 chars — structured errors the model must answer for, no proposal from invalid input); propose_payment_reminder → the CANONICAL repos.debt.sendReminder execution leg (AI-308 residual b closed); the honest settle for unexecutable types.
  - T-268 (20a2ceb): the clarification question card (parsed from request_user_clarification results, answered as a framed turn), the abort button (AbortController per turn → the runtime's signal; partial answer COMMITTED + info toast), conversation persistence (localStorage, 60-message cap, corrupt-safe).
  - T-269 (2cbbace + the EF deploy): ai-proxy hardened (`.maybeSingle()` on the tenant-config lookup — the OPS-009 class; agent-mode input validation: model-id pattern, role enum, content type, 60-msg/20-tool caps); **GROQ_API_KEY set live** (CLI timeout quirk; digest + behavior verified); EF deployed v20; **the LIVE agent-stream round-trip verified for the first time** (9/9 probes: anonymous 401 ×2, invalid_model/invalid_role 400 ×2, the authenticated SSE round-trip with real French content, the tool-call passthrough — `search_entities({"query":"Benali"})` reassembled on the wire, the single-shot narrative path with real tokens/latency); **the 2026 Groq catalog discovery** — every llama/qwen id 404s with the owner's key → defaults repinned everywhere to openai/gpt-oss-120b (default+reasoning) / openai/gpt-oss-20b (fast+fallback); the gpt-oss reasoning-channel behavior documented (delta.reasoning + channel:"analysis" shares the max_tokens budget; the desktop parser correctly ignores it). Evidence: docs/recovery/t-269-live-verification.md + scripts/t269-live-matrix.sh.
  - T-270 (3a1167b): NEW 31-test deep-integration suite (tools against the REAL mock repositories; all five validation failure shapes; clarification/abort/persistence through the REAL runtime with stubbed SSE; the send_reminder execution leg; source guards pinning the restored architecture). Harness lessons documented in-code: the stale-closure trap, the locked-ReadableStream trap, the spread-spy trap (class prototype methods vanish under a spread).
  - T-271 (this commit): ADR-015 (the copilot data policy: grounded tool results, BYOK/EF transports, human-gated writes, honest settle, LIVE-reachable defaults), registries, this change-log entry, current-state 41st snapshot, next-task 42nd recommendation, zips + push.
- **Migration-token verification (the owner's explicit ask):** chain 81/81 = 0001–0084 **ZERO DRIFT** at session open AND close; EF fleet 14/14 ACTIVE (ai-proxy redeployed v20); the supplied tokens verified live (sbp_ access token, service-role, anon); no migration was needed (the session's changes are EF + client code — deliberately DB-free). The admin password (rotated by the owner since T-241) was re-reset to the documented value following the established runbook to enable the authenticated probes — the owner should rotate it again.
- **Tests:** full desktop suite **119 files / 2824 / 0** (+31 vs the 40th close's 117/2789; the session opened at 5 failed / 2787 — the REG-005 damage, repaired at T-266) + tsc 0 errors + eslint 0 errors + vite build green.
- **Verification:** live evidence above + the suite; nothing marked TESTED/VERIFIED without recorded evidence.
- **Commits:** a461227, c7107a7, 20a2ceb, 2cbbace, 3a1167b + this closeout.
- **Notes / residuals:** (a) the DB permission catalog still lacks use_ai for non-SuperAdmin roles (AI-309 residual — needs a migration if the school ever adds AI-authorized non-admin roles); (b) record_attendance / dispatch_task remain unexecutable BY DESIGN until canonical paths exist (ADR-015); (c) the EF's ai_provider_configs ciphertext is still not decryptable server-side (env-secret fallback is the documented path); (d) the copilot conversation is client-persisted only (ADR-015); (e) the Groq key was transmitted in chat — owner hygiene: rotate it in the Groq console at leisure (it is stored ONLY as the live EF secret, never in any repo).

---

### 2026-09-09 — 38th session (T-255..T-259) — the Power BI-style ANALYTICS report page delivered (UI-307): 9 visual families + interactive cross-filtering slicers, all REAL-data derivations

- **Problem IDs:** UI-307 (NEW this session, TESTED same-session)
- **Mandate:** owner feedback hours after the 37th-session push — "You didn't add enough graphs. I wanted more Power BI–type visualizations, with things like interactive charts, statistics, comparisons, trends, and other data visualizations wherever they make sense."
- **What changed (hub repo — desktop):**
  - T-255 The Analytics tab (`tabs/analytics-tab.tsx` + the "Analytique" PageTab in dashboard-page.tsx): a 12-column report page — slicer bar → 6-card statistics strip → hero trend explorer + method/category mix pair → YoY comparison + amount histogram → collection heatmap + aging composition → top-debtors Pareto. The T-088 single-fetch pipeline preserved (the tab never fetches; the page adds ONE aggregate call — the previous-year `revenueForRange` over the one-year-shifted month window, leap-day safe, only for years in AVAILABLE_ACADEMIC_YEARS).
  - T-255 The derivation layer (`components/analytics/analytics-derivations.ts`, pure + fully exported): the ENCAISSÉ consistency contract (status=paid + calendar month of collectedAt — the repository revenue series' own definition, so the analytics cards reconcile with the KPI cards; the weekly-rhythm's skip-refunded counter-activity convention is documented as a deliberate difference); `applyAnalyticsFilters` (the cross-filter engine: paid + range + method + category, empty selection = ALL, additive multi-select); `derivePaymentStats` (count/total/mean/median/σ/min/max/best-month); `deriveRevenueTrend` (cumulative + 3-month MA, null until the 3rd point); `deriveFilteredMonthly` (the overlay aligned to the series' labels); `shiftIsoYearBack`/`previousAcademicYear`.
  - T-256 The mixes + matrix: method donut with the REAL center total + callout legend; category ranking (horizontal bars, [Montant | Nb opérations] metric toggle, top-6 + "Autres (n)" tail); the weekday×month collection heatmap (CSS-grid matrix — Dim→Jeu school week, Fri/Sat excluded per the T-243 convention, 5-level intensity quantized against the matrix max, year-aware tooltips, row/column Σ, intensity legend); the amount histogram (fixed DZD bins, dominant-bin verdict).
  - T-257 The recouvrement analytics: top-debtors Pareto (desc bars + cumulative % line on a right 0–100 axis + the derived "N familles = 80% de l'encours affiché" verdict); aging composition (100% stacked AGING_COLORS bar + per-bucket table with total row); the like-for-like YoY comparison (grouped N vs N−1 bars per month, header Σ + global delta, delta NULL where the previous window has zero activity — a divide-by-zero is not a −100% trend — and the honest unavailable state for the earliest selectable year).
  - T-255 The slicer bar (`analytics-slicers.tsx`): method chips (3 canonical) + category chips (only categories PRESENT in the period — no inert chips) + live badge ("N / total opérations · Σ DZD") + Réinitialiser. Cross-filters EVERY payments-derived card (stat strip, mixes, heatmap, histogram) + the dashed "Filtré" overlay on the trend chart — the canonical repository bars/lines are never silently replaced (§15.16).
- **Tests:** NEW `src/tests/ui/analytics-visuals.test.tsx` (34 tests: 26 pure-derivation tests — the slicer matrix incl. paid-only + range + method/category + empty-selection semantics, statistics incl. median parity/σ/best-month/empty, MA3 null-until-3, overlay label alignment, mix percents + tail merge, heatmap month columns + school-week rows + Fri/Sat exclusion + level quantization + month totals, histogram bin edges incl. the 5k boundary, Pareto cumulative + zero-drop, aging shares + canonical order, YoY null-delta, the leap-day shift; 8 render tests — the full chart wall with REAL derived figures, the method + category chip cross-filtering with the live badge + reset, the "Filtré" overlay appearing only under active filters, the Barres↔Aire view toggle, heatmap cell data attributes, aging aria-valuenow shares, 7 honest empty states). Recharts mocked (the T-247 jsdom pattern); ResizeObserver stub local (the global setup untouched).
- **Verification:** desktop **114 files / 2745 tests / 0 failures** (+34 vs the 2711 baseline) + `npm run typecheck` clean + `npm run lint` 0 errors (410 warnings, +3 from the test's ResizeObserver stub — the established T-247 pattern) + `npm run build` green (20.7 s). Session-opening ritual honored AFTER the container reset: the 3 repos re-cloned (hub 2ad3b17 / website f3d9965 / android f0b6433 — all pushes were confirmed so nothing was lost), npm re-provisioned, and the pristine-tree baseline re-proven **113 files / 2711 / 0 + tsc + eslint 0 err (407 warn) + build green** before any change. Website + Android untouched (desktop-scope UI change, zero contract impact per §10). Zero migrations — the live chain 0001–0084 and the 37th session's 18/18 live evidence stand unchanged (no live round needed: no backend surface was touched).
- **Commit:** see the git log (feat(ui) T-255..T-258 + this docs closeout).
- **Notes:** residuals registered in the UI-307 entry: KPI sparklines beyond revenue still need repository history; YoY limited to AVAILABLE_ACADEMIC_YEARS (the earliest selection renders the honest unavailable state); revenueByCategory/revenueByCycle remain the backend residuals that would extend the analytics surface further; the heatmap caps at 24 month columns for degenerate custom ranges. The dashboard's other surfaces keep their focused visuals (the Overview keeps the 3-zone layout, the drill-down keeps the detail views — UI-306's placement decisions are unchanged); the Analytics tab is the analytical surface. Owner hygiene note carried over: rotate the chat-transmitted GitHub PAT when convenient (AGENTS.md rule 12 / SEC-100 class).

---
### 2026-09-09 — 37th session (T-246..T-254) — the AI-review's REMAINING screens + Part-2 design system delivered (UI-306 continuation); live migration-token round 18/18 GREEN

- **Problem IDs:** UI-306 continuation (TESTED same-session)
- **Mandate:** owner re-supplied the same AI-review package ("do all of this then push") — the 36th session had delivered only the dashboard Overview (T-243) + the token round (T-244); this session completed the review's Part-2 design system and its Screens 1–2/4–8, re-verified the live backend, and packaged everything.
- **What changed (hub repo — desktop):**
  - T-246 Design system: index.css midnight canvas (#0b0d14/#111523/#181d30 + --surface-hover), crisper status hues, --brand-violet/--brand-coral, .glow-border micro-rim; tailwind.config.cjs hex parity + the TS token-mirror fallbacks (dashboard-theme chartPalette, AGING_COLORS, STROKE_BY_TONE) — the PARITY CONTRACT is now documented in index.css itself; light theme gains darker status variants. The personnel COLOR_TO_HEX DB registry deliberately untouched (owner-gated data decision — recorded in the UI-306 continuation).
  - T-247 SeeDetailsModal: collection-rate trio (Encaissé annuel / Créances restantes / Taux de recouvrement), the "Encaissements vs échéancier théorique" ComposedChart (bars REAL + the DERIVED 40/30/30 planning line over encaissé+créances, omitted when kpis null), the dual-ring donut with the REAL center total + legend callouts, per-bucket severity badges, kpis typed, chart chrome unified on DASHBOARD_THEME.
  - T-248 InstallmentScheduleTab: the Tranche Wave header — T1/T2/T3 collection health from REAL filtered rows via the canonical `trancheNumberOf` matcher + canonical sum helpers; next-target highlight; honest zero state.
  - T-249 UnifiedPaymentModal: quick-pay shortcut chips (≤2 open tranches + Soldé total) + the cashier change-return calculator (Montant remis → Monnaie à rendre, Espèces-only, reset on close). Collect path untouched.
  - T-250 TuitionCard: the 14-card scroll-wall → ONE consolidated matrix (Équilibre column, dirty dots, save blocked while unbalanced) + the EXPLICIT 40/30/30 tool (never auto-applied — the official grid is non-equal); canonical due-month labels (15 sep/15 déc/15 mars).
  - T-251 AcademicYearDrawer: the "État de Préparation" readiness checklist (classes/homeroom/timetable REAL rates) + the full 14-level grid with honest empty tiles.
  - T-252 ParentDetailDrawer: the family coverage stack (clearedPaid/netDue, pending legend, bridge preserved) + per-tranche 1-click collect (real row → exact installment_tranche context with canonical INV-4 remaining; synthetic → consolidated preset, no phantom targetItemId); the drawer's collect modal is now context-state-driven.
- **Tests:** NEW src/tests/ui/ai-review-screens.test.tsx (17 tests: projection math incl. rounding/0-expected, the Tranche-10/Année-complète matcher negatives, wave grouping/sums/next-target/pending, summary trio + 53% rate, projection-line kpis gating, donut REAL center, severity badges, quick-pay chips, change-calculator interaction 40000−32000→8000, 14-level matrix, drawer Encaisser buttons on the seed's real open tranches). jsdom ResizeObserver stubbed locally (PageTabList — first test to render it; global setup untouched).
- **Verification:** desktop 113 files / 2711 / 0 (+17 vs baseline 2694) + tsc clean + eslint 0 errors (407 warnings) + vite build green (19.1 s); website re-verified untouched-green 40 files / 544 / 0 + lint + strict build; Android untouched (desktop-scope UI change, zero contract impact per §10). T-253 live round 18/18 GREEN (script /home/z/my-project/scripts/verify_t-253_mig_tokens.sh — the container reset wiped the scripts dir; CLI re-provisioned v2.116.0): dual-key health 200 ×2, RLS anon 0-rows ×5, chain live=81/local=81 max 0084 ZERO DRIFT, EF fleet 14/14, anonymous EF 401 ×2, ALLOWED_ORIGINS 4-origin echo + non-allowlisted rejected, committed-key consistency (website public-config + Android .env.example). No apply needed.
- **Commit:** see the git log (feat/design + test commits per task batch).
- **Notes:** residuals registered in the UI-306 continuation: category pills need a backend revenueByCycle; the Departments per-category breakdown needs DashboardRepository.revenueByCategory(); admissions pipeline unchanged; the personnel hex registry is an owner-gated data decision. Push addendum (same day): the owner re-supplied a valid PAT — 4d28a79 + 2632cfd pushed to origin/main (receipt 3e676a8..2632cfd) and the post-push fetch confirms all three repos in-sync (zero delta); zips remain in download/.

### 2026-09-06 — 31st session (T-199..T-208) — the mobile-viewport UI defect family: live-measured, fixed, guarded, and cross-platform-paritied; live full-stack consistency re-proven

- **Problem IDs:** UI-300, UI-301, UI-302, UI-303, UI-304 (all NEW this session, all TESTED same-session) + T-204's verification round (no new problems — all probes green)
- **Mandate:** owner's "fix the website UI (elements overflowing containers, not fitting; mobile-first but desktop must work) — not just UI patches: fix the underlying problems across the stack including the Supabase backend; apply/verify the migration tokens consistently" — selected a balanced 10-task set (T-199..T-208).
- **What changed (website repo — 6 commits):**
  - T-199 UI-300 (Critical): dashboard two-column grid gained the base `grid-cols-1` (a bare `grid gap-6 lg:grid-cols-2` left an implicit minmax(auto,auto) track below lg that sized to the CardListItem max-content text — 880px of horizontal page overflow at 375px, 935px at 320px). NEW guard `t-199-grid-blowout-guard.test.ts` (2 tests: the fix + a whole-src scan of the bug class).
  - T-200 UI-301 (High): KpiCard values (`text-2xl` mono, Intl U+202F narrow-no-break-space groups → one unbreakable token) poked 108px out of mobile cards. Display-layer fix: `break-words` + `text-xl sm:text-2xl` + icon `hidden sm:block`. NEW guard `t-200-kpi-value-overflow.test.ts` (4 tests incl. the formatter-parity pin).
  - T-201 UI-302 (Medium): four non-wrapping header rows (finance 163px/academic 77px/notifications 39px/documents-card 42px @320) → one fix class (`flex-wrap` + `gap-y` + `min-w-0` titles). NEW guard `t-201-header-wrap.test.ts` (5 tests).
  - T-202 UI-303 (Medium): financial 5-tab bar clipped labels at ≤360px (54px cells vs 58–84px labels, whitespace-nowrap) → scrollable chip row below sm + equal grid at sm+ (the established `overflow-x-auto scrollbar-none` idiom) + basis/shrink neutralization. NEW guard `t-202-tabs-mobile.test.ts` (3 tests).
  - T-203 UI-304 (Low): dashboard rendered raw English kind enums ("meeting"/"reminder") while calendar-view held the canonical mapping → extracted `src/features/calendar/event-kind.ts` (kindToUiType + uiTypeLabelKey/eventKindLabelKey); both views consume it. NEW guard `t-203-event-kind-labels.test.ts` (4 tests incl. the duplicate-map guard + dictionary completeness).
  - T-207 website half: AGENTS.md §5 — the five mobile-first layout rules with their guard suites.
- **What changed (hub repo):** the registry registrations (UI-300..304 + T-199..208 upfront, committed BEFORE implementation per the workflow), the per-task registry checkouts, T-204's evidence doc `docs/recovery/t-204-live-verification.md`, T-205's desktop parity fix (18 files: base `grid-cols-1` on every bare responsive grid — 6 found by grep + 12 more the new whole-src guard caught on its first run — + `break-words` on the debt KPI value; NEW `src/tests/ui/t-205-grid-blowout-desktop.test.ts` 2 tests), T-207's `docs/testing/strategy.md` §6 (the live UI-verification harness recipe) + hub AGENTS.md §15 rules 21/22, and this closeout.
- **Why:** the portal is mobile-first by design but had never been layout-verified at real phone widths; the audit families (grid blowout, unbreakable currency, non-wrapping headers, tab clipping) were all live-measured DOM geometry, not guesses. The desktop instances were latent (Electron minWidth 1100 > lg 1024) and fixed for parity per §10; Android was scanned and found structurally immune (Compose constrained layout + `weight(1f)` + Text softWrap — Kotlin `formatDzd`'s U+00A0 groups cannot blow out the screen) — no divergence registered, no Android changes.
- **Verification:**
  - Live UI harness (seeded UI-TEST family in the live DB + @supabase/ssr cookie sign-in): **viewport matrix 36/36 GREEN** — all 9 views × 320/375/768/1280 at 0px document overflow (dashboard was 935/880/487/0 before); the 4 financial sub-tabs + the messages conversation 0px at 320; the 5-tab bar renders every label unclipped (92/78/86/61/100px) in a scroll row; dashboard pills render "Réunion | Échéance" (was "meeting | reminder").
  - Website suite growth: 30 files/496 (30th close) → **35 files / 514 tests / 0 failures** (+5 guard files, +18 tests); lint clean; strict build green.
  - Desktop suite: 90/2410 → **92 files / 2422 / 0** (+1 guard file, +12 tests); typecheck clean; lint 0 errors.
  - Android: UNCHANGED this session (zero commits) — the 30th session's 48/410/0 evidence stands for the identical tree; no Android code was touched (the parity scan was read-only).
  - T-204 live consistency round (probes only): chain **76/76 = 0001–0079 zero drift**; auth health + REST 200 with BOTH key formats (ADR-009 dual acceptance re-proved); **EF fleet 13/13 ACTIVE, 1:1 with the hub's 13 function dirs**; the canonical 4-origin ALLOWED_ORIGINS all echo on preflight (ACT-203 intact) and a non-allowlisted control does NOT; anonymous bind-activation-code POST → 401.
  - UI-TEST family cleaned from the live DB post-verification (parents/students/ledger/payments/installments/attendance/homework/calendar/notifications/chat/personnel/profiles/auth-users all 0 residual; audit_logs rows kept per the append-only convention).
- **Discoveries (all persisted):** the "@supabase/ssr browser client stores sessions in COOKIES, not localStorage" sign-in recipe; the Next dev-tools `<nextjs-portal>` overlay covers click points (remove before interacting); Radix tabs respond to focus+Enter, not el.click(); every live check constraint needed for the seeded family (payments transfer needs proof_path per 0045; ledger account_id is the composite parent:uuid:category:cat:student:uuid; attendance/notification/calendar/chat enums); **the prior sessions' "EF fleet 14/14" figure was STALE — 13 live = 13 hub source** (the 14th was a since-removed drifted website EF copy); the Functions API returns status "ACTIVE" UPPERCASE (a lowercase jq filter counts 0). Rules 21/22 added to AGENTS.md §15; strategy §6 documents the full recipe.
- **Commits:** website 6 new (registration-then-fix sequence: T-199, T-200, T-201, T-202, T-203, T-207-website) + hub 8 new (registration, T-199..T-207 checkouts + T-204 evidence + T-205 code + T-207 docs) + this closeout. Full hashes in each repo's git log.
- **Notes:** scope was held to the 10-task charter — no financial write paths, no RLS, no migrations (the chain stayed 76/76 all session); production data untouched (the UI-TEST family was tagged and is provably removed). **Owner residuals unchanged:** FIREBASE_SERVICE_ACCOUNT_JSON (real FCM sends), RESEND_API_KEY (workflow emails), the website web-push Firebase env vars, AUTH-200's Google OAuth client.
- **Next:** the actionable OPEN set remains owner-gated/device-gated (see next-task.md); the next agent should re-run the matrix script after any layout change (strategy §6).

## Entries

### 2026-09-10 — 40th session (T-264..T-265) — the post-39th-launch 406 console-noise fix (OPS-309): the approval-queue matcher goes .maybeSingle(); the owner's Groq BYOK key geo-blocked from the verification sandbox

- **Problem IDs:** OPS-309 (NEW this session, TESTED same-session)
- **Mandate:** owner report at the 39th-session production launch (2026-09-09 20:36 console, pasted into chat): "okay then fix it and push it" — the ~15 repeated red 406s on every approval-queue refresh (`…bound_to_auth_user_id=is.null → 406`, `…auth_user_id=is.null → 406`) + one 500 on an audit-log list query. A Groq BYOK key was supplied "in case" for the AI.
- **What changed (hub repo — desktop):**
  - T-264 / OPS-309: `src/infrastructure/supabase/repositories/supabase-approval-repository.ts` — all FIVE `.single()` calls in `findPotentialMatches()` converted to `.maybeSingle()` (activation_codes by code; parents by id / email / national_id / phone). Zero rows is the COMMON case for these lookups (most pending web registrations have no matching parent yet), and `.single()` turns 0 rows into a PostgREST 406 PGRST116 error — the app worked (only `data` is destructured; error-or-null both fall through to the next strategy) but every refresh spammed the console. `.maybeSingle()` returns `data:null, error:null` for 0 rows (silent HTTP 200); the >1-row case still errors and still falls through — enrichment semantics byte-identical. The four `.is(..., null)` not-yet-bound filters preserved verbatim (binding semantics, not noise). Doc comment added at the method (T-264/OPS-309 provenance).
  - NEW `src/tests/infrastructure/t-264-approval-406-noise.test.ts` (7 tests — 5 behavioral + 2 source guards): zero-row lookups surface NO error (listPending Ok, `parent_match: null`, the email strategy walked with `auth_user_id: null` filter recorded); the activation-code canonical path still enriches; a >1-row maybeSingle error still falls through; the email fallback still enriches after a code miss; an EMPTY queue fires zero matcher lookups; source guards (no `.single();` call site remains — 5 `.maybeSingle();`; the null-filters preserved) per the t-126 scan precedent.
  - T-265: this closeout (registries + push with the owner PAT, one-shot URL).
- **Why:** the owner pasted the production console at the 39th-session launch; the 406s were traced to the approval matcher's zero-row `.single()` lookups (the 500 on `.desc&limit=200` was a ONE-OFF audit-log list failure at startup — no repro this session, not chased per §15.8's one-problem-per-task rule; noted in next-task as an observation).
- **Affected components:** desktop approval-queue enrichment only (Settings → Approvals tab's `listPending()`); no schema, no migration, no EF, no Android/website code (desktop-scope, zero contract impact per §10; the live chain 0001–0084 and the 39th session's live evidence stand unchanged).
- **Tests:** `npx vitest run src/tests/infrastructure/t-264-approval-406-noise.test.ts` → 7/7; full `npm test` → **117 files / 2789 tests / 0 failures** (5 skipped, pre-existing; +1 file / +7 tests vs the 39th close's 116/2782) — run TWICE (once before a type-only fixture fix, once after; same result); `npm run typecheck` clean; `npm run lint` 0 errors (424 warnings, unchanged count from the 39th close — the new test file adds none); `npm run build` green (19.85 s).
- **Verification:** suite-level (above). Live backend NOT touched this session (no migration, no EF change — the 39th session's 7-probe matrix evidence stands). The owner-supplied Groq key (gsk_…, transmitted in chat) was NOT verifiable from this sandbox: api.groq.com returns HTTP 403 `{"error":{"message":"Forbidden"}}` on BOTH /models and /chat/completions — traced to the sandbox's egress region (Hong Kong, Alibaba AS45102), a Groq-geo-blocked territory; this is NOT evidence the key is invalid (a malformed key returns 401). The key must be validated from the owner's machine — the desktop Settings → AI tab's live model discovery does exactly that in one click. Per §15.12 the key was never written to any repo file, script, or doc; only its STATUS (unverifiable-from-sandbox) is recorded here. The EF server-side GROQ_API_KEY secret (AI-308 residual d) remains owner-gated: setting it needs the sbp_ access token (not supplied this session) or a dashboard manual set.
- **Commit:** see the git log (fix(desktop) T-264 + this docs closeout).
- **Notes:** enrichment semantics deliberately preserved (only the transport-level 0-row representation changed); the >1-row edge behaves as before (error → fall-through). Owner hygiene: BOTH credentials transmitted in chat this session (the Groq key + the GitHub PAT) — rotate both when convenient (the PAT rotation is already a standing owner-gated residual; the Groq key should be rotated at console.groq.com after confirming it works, since it is now in a chat transcript). Terminology note recorded for the owner: the supplied key's `gsk_` format is a GROQ key (the provider the copilot supports); "Grok" (xAI) keys use the `xai-` format and the `https://api.x.ai/v1` endpoint — usable via the copilot's custom_openai provider if that was the intent.


### 2026-09-10 — 39th session (T-260..T-263) — the Agentic AI Architecture delivered (AI-308): streaming + tool-calling copilot with BYOK live model discovery and human-in-the-loop action proposals; ai-proxy EF deployed live

- **Problem IDs:** AI-308 (NEW this session, TESTED same-session with suite + live-verify evidence)
- **Mandate:** the owner's Agentic AI Architecture blueprint + credential sheet ("do all of this then push"): a model-agnostic, autonomous, production-ready AI agent for El-Imtiyaz — Groq/OpenAI-compatible providers with native streaming (`stream: true`) and tool calling; BYOK keys AES-256-GCM-encrypted with live model listing via the provider's `/models` endpoint and zero hardcoded model locks; deep domain grounding (rosters, ledger, attendance via the exact `src/domain/calc/` engines); human-in-the-loop action proposals with 1-click validation; a universal copilot via Ctrl+J / topbar.
- **What changed (hub repo — desktop):**
  - T-260 The agent core: `src/domain/model/ai.ts` extended (custom_openai provider + sampling params + the AIModelInfo/AIChatMessage/AIToolCall protocol; every legacy export preserved — baseline contract) · NEW `src/core/ai/agent-types.ts` (ToolDefinition + ActionProposal, `requiresApproval: true` invariant) · NEW `src/core/ai/streaming/stream-client.ts` (SSE parser with indexed tool-call fragment reassembly) · NEW `src/core/ai/providers/provider-registry.ts` (3-provider endpoints + `queryLiveProviderModels`) · NEW `src/core/ai/tools/system-tools.ts` (6 domain tools executing against the REAL repositories + canonical calc engines; names via the canonical display-name renderers; adjustments PROPOSE only) · NEW `src/core/ai/agent-runtime.ts` (multi-turn loop, MAX 5 tool rounds, abortable) · `ai-config-storage.ts` (3 encrypted keys + sampling; legacy payloads load with defaults) · `llm-adapter.ts` (BYOK leg on the shared streaming client, 3-provider chain, custom never falls back to cloud; SEC-002 single-shot policy verbatim) · mock ai-config-repository merges the new fields.
  - T-261 The universal copilot: NEW `src/app/providers/ai-copilot-provider.tsx` (INSIDE ToastProvider — the blueprint's provider order would throw; approveAction = the ONE write path, repos.payments.adjust with the session actor) · NEW `src/features/ai/copilot-drawer.tsx` (live streaming bubble, tool chips, Valider/Ignorer proposal cards; zero direct repository writes) · `topbar.tsx` (Assistant IA button UseAI-gated + Ctrl+J/Cmd+J + palette Enter-to-ask + the always-on ask-the-AI row — additive integration, the blueprint's wholesale topbar replacement REJECTED per §15.3) · `app.tsx` provider wiring · `app-shell.tsx` drawer in both branches · `ai-config-tab.tsx` rebuilt (live model discovery, streaming inference test with latency, custom base URL, sampling sliders; RBAC + audit persistence preserved per §15.4) · i18n fr+ar keys.
  - T-262 The EF: `supabase/functions/ai-proxy/index.ts` agent-stream mode (messages + tools + model override + top_p → provider with stream:true → SSE piped back verbatim; auth/permission/rate-limit/audit machinery preserved; request row logged before the stream; `logAgentRequest` helper) — deployed live v17, no migration (chain stays 0001–0084).
- **Tests:** NEW `src/tests/ai/agent-architecture.test.ts` (29 tests — domain model + legacy-export survival, SSE parsing [content + tool fragments + [DONE]/keep-alive/malformed + HTTP error + wire shape], registry [endpoints + custom URL + both /models shapes + error], tool schemas, executeSystemTool ×8 against the real mock repositories, the runtime [full tool round, MAX-5 guard, missing-key guard, wire body], storage round-trip + legacy load) + NEW `src/tests/ai/copilot-drawer.test.tsx` (8 tests — drawer render, Driver canUse gating, the missing-key error toast, approveAction → payments.adjust, 4 source guards).
- **Verification:** desktop **116 files / 2782 tests / 0 failures** (+37 vs the 2745 baseline) + `npm run typecheck` clean + `npm run lint` 0 errors (424 warnings, +14 from the new files — warnings-only, the 0-error gate holds) + `npm run build` green (21.1 s). LIVE: ai-proxy deployed (v17, ACTIVE) + the 7-probe matrix GREEN — OPTIONS 200 / GET 405 / POST-no-auth 401, EF fleet 14/14 ACTIVE, chain live 81 = local 81 ZERO DRIFT (max 0084), RLS anon 0-rows (200), GoTrue health 200 — evidence in `docs/recovery/t-262-live-verification.md`. Website + Android untouched (desktop-scope + an additive, backward-compatible EF change; zero contract impact per §10).
- **Commit:** see the git log (feat(ai) T-260..T-262 + this docs closeout).
- **Notes:** the blueprint adaptations (each registered in the commit + the AI-308 entry): provider INSIDE ToastProvider; topbar integration additive (blueprint replacement rejected — §15.3); RBAC gates preserved on the settings tab + UseAI on the copilot (blueprint's un-gated variant rejected — §15.4); the copilot conversation is deliberately unmasked domain-grounded data via tools (the single-shot feature paths keep the SEC-002 masked-content policy verbatim); custom_openai never cross-falls-back to a cloud provider; adjustments only ever PROPOSE (approveAction performs the canonical write). Residuals in AI-308: live authenticated agent-stream round-trip (needs staff JWT + server key), non-financial proposal dispatch, conversation persistence, abort button wiring, Android/website copilot equivalents.
...
- **Commit:** hub + desktop `bca2d1a` (T-185/T-186/T-188 — one atomic repo state); website `6bbdb33` (T-187).
...
- **What changed:**
  1. **Website** — `src/features/auth/activation-code-screen.tsx`: the EF URL + apikey now resolve through `@/lib/env` (T-096 fallback chain: build-time env vars → committed `PUBLIC_CONFIG_DEFAULTS`) instead of DIRECT runtime reads of the build-time env vars. New regression suite `src/test/t-184-activation-ef-url.test.ts` (7 tests) with a whole-src scan that fails if any non-`env.ts` file ever reads `process.env.NEXT_PUBLIC_*` directly. Permanent rule added to the website `AGENTS.md` §5. (The T-153 `mapActivationError(data)` contract untouched.)
  2. **Desktop** — `src/infrastructure/supabase/repositories/supabase-approval-repository.ts`: NEW `structuredEdgeFunctionError()` helper (parses the hub EF's `{error:{code,message}}` body off `FunctionsHttpError.context`) + `bindActivationCode` maps the EF codes to precise AppErrors (already-active/already-bound → conflict, not-found/expired → validation with the real message, suspended/rejected → forbidden; network/no-context/non-JSON fall back to the unchanged generic mapping). New regression suite `src/tests/infrastructure/t-184-bind-activation-structured-errors.test.ts` (10 tests).
  3. **Docs** — ACT-201 + ACT-202 registered (problem-registry totals 183→185, TESTED 143→145) + the ACT-201-related owner note (the Firebase env warning in the same paste = the documented truthful T-121/AUTH-202 state, NOT a defect); task-registry T-184; this entry; website AGENTS.md rule.
- **Why:** the owner's production console showed `/undefined/functions/v1/bind-activation-code` → 404. Root cause: Next.js inlines `NEXT_PUBLIC_*` values at BUILD time; the production Vercel project sets none (the same paste's FCM warning proves it), so the screen's direct env read inlined `undefined` → the browser requested a RELATIVE path on the PORTAL origin → 404 from the portal itself, on EVERY activation attempt. Live probes taken BEFORE fixing (publishable-key apikey POST → the EF's own structured 401; OPTIONS → 200; garbage Bearer → 401 auth_failed) proved the deployed EF, the T-145 issuance path and the T-146/T-147 semantics all healthy — the ENTIRE regression was the client URL construction. The owner explicitly ordered this fix to jump ahead of the in-progress 28th-session batch (T-176+).
- **Affected components:** website activation screen (+ test + AGENTS.md); desktop approval repository (+ test); hub registries. NO EF, migration or Android changes (all verified healthy — Android's activation path is server-issued codes + portal binding, unaffected).
- **Tests:** website `npx vitest run` full = **28 files / 483 / 0** (was 27/476) + `npm run lint` clean + strict `npm run build` green; desktop new suite **10/10** + t-145 **5/5** + t-146 **8/8** + FULL suite **86 files / 2371 / 0** + `tsc --noEmit` clean + lint 0 errors (suite count includes T-176's in-progress workflow tests — separate commit).
- **Verification:** live URL-routing proof: the exact request the fixed client emits (POST + publishable-key apikey, no auth) now reaches the REAL EF → `{"error":{"code":"unauthorized"}}` 401; simulated OLD construction reproduces the owner's exact URL `"undefined/functions/v1/bind-activation-code"`. The authenticated happy path keeps T-147's standing 19/19 live round-trip as evidence (server side unchanged).
- **Commit:** (this commit — website + hub, one per repo per the git standard)
- **Notes:** the full happy-path (real parent + real code) remains owner-device-gated exactly as ACT-200 documented — but the blocker that made it fail for EVERY code regardless of validity is gone. **Owner action: redeploy the portal on Vercel** (code-level fix, no env vars required; setting `NEXT_PUBLIC_SUPABASE_URL`/`NEXT_PUBLIC_SUPABASE_ANON_KEY` as Vercel env vars is optional hardening). The 28th-session batch (T-176 workflows port — files staged in the working tree, tests green → T-177..T-183) RESUMES immediately after this commit.
...
### 2026-09-05 — T-174 — MIG-TOKENS 28th-session re-verification + 0069 live registration repair (ARCH-015)

- **Problems:** ARCH-015 (new — 4th ARCH-011-class event: migration 0069's DDL was applied live by the 26th session but its schema_migrations registration row was never inserted; live chain read 65 rows vs 66 committed files)
- **What changed:** `scripts/apply_0069_registration_live.sh` (NEW — registers the 0069 row atomically, T-091/MIG-TOKENS pattern, env-token, ON CONFLICT DO NOTHING; the committed 0069 file was intentionally NOT edited — the T-058 machine-enforced append-only guard forbids touching an existing migration file, and fresh CLI deployments register automatically); `scripts/verify_t-174.sql` (NEW — the 5-check live verification); Supabase CLI 2.116.0 re-provisioned at /home/z/my-project/bin (the container reset wiped it); problem-registry ARCH-015 entry; credentials.md §7 re-run recorded.
- **Why:** the owner's mandate "apply the migration tokens… the migration is properly applied and remains consistent everywhere" — the opening chain check is the detector, and it found the drift.
- **Live verification:** apply 201 → chain **67/67 = 0001–0070 ZERO DRIFT** (after 0070 below); `verify_t-174.sql` **5/5** (chain=66-at-apply-time, 0069 row shape exact — statements `{0069_adjustment_description_guard.sql}` + name `adjustment_description_guard`, the constraint still convalidated, unique version index → structural idempotence, 690 adjustment rows untouched). §7 credentials checklist **17/17**: auth health 200 ×2 key formats (legacy anon JWT + sb_publishable), RLS anon/publishable deny ×5 core tables ×2 keys (0 rows), owner-supplied URL/publishable/JWKS byte-identical to the committed values (website public-config.ts, Android .env.example), JWKS endpoint 200. EF fleet census: **13/13 ACTIVE** (ai-proxy, approve-signup-request, bind-activation-code, collect-payment, create-user-account, expire-pending-approvals, purge-expired-backups, refresh-materialized-views, refund-payment, run-overdue-scan, send-push-notification, update-server-secret, workflow-execute) — all local function dirs deployed; earlier "14/14" counts included `_shared/` (a module dir, not a function).
- **Baselines (pristine clone + session work):** desktop **84 files / 2 348 / 0 failures** + typecheck clean + lint 0 errors/392 warnings; website + Android re-run at closeout (see T-183).
- **Lesson (compounds ARCH-011/013/014):** a Management-API apply that runs only the migration file's SQL — even when the file is COMMITTED — silently under-registers the chain; the registration INSERT must ride in the same atomic payload. The 26th session's "applied live: convalidated=true" was DDL-true and registration-silent.
- **Commit:** (this session's hub commit hash)

### 2026-09-05 — T-175 — Desktop: calendar port → calendar_events (T-047 port #1, migration 0070)

- **Problems:** ARCH-001 (the T-047 #1 port per the T-160 scoping: the only slot with a verified live cross-platform read — the website already queries calendar_events while desktop wrote to the in-memory mock store)
- **What changed:** migration **0070** (`0070_calendar_priority_assignment.sql` — priority/assigned_to_user_id/assigned_to_role columns; the T-160 "no schema work needed" held at table granularity but not column granularity: the desktop creator modal's Priorité select + assignment fields had no columns) applied live atomically (`apply_0070_live.sh`, chain → **67/67**); NEW `SupabaseCalendarRepository` (month-bucketed SubjectBehavior cache with T-034/CROSS-104 freshness + per-bucket seeding; four-source merge: manual rows + derived payments (paid/partial, joined parents) + audit_logs (auth noise skipped) + expense milestones; create/update/delete with soft-delete + auto-kind rejection = mock parity); wired into `getSupabaseRepositories()`.
- **Why:** pre-T-175 the desktop calendar was the ARCH-006 pattern: staff-scheduled events lived in memory only (wiped on restart, invisible to the website) AND the derived daily-activity events were computed from mock SEED data while real payments/audit/expenses lived in Supabase.
- **Tests:** NEW `src/tests/infrastructure/supabase-calendar-repository.test.ts` **12/12** (full create mapping ×4 kinds, all-day convention, auto-kind rejection, update re-derivation, soft-delete + bucket eviction, four-source merge + sort + derived shapes, status/date filtering, expense milestone month-splitting, persistence-across-restart, wiring source-scan, cross-platform kind parity: domain union == migration 0013 CHECK == website CalendarEventRow). FULL suite **84 files / 2 348 / 0 failures**; typecheck clean; lint 0 errors (392 warnings vs 381 baseline — +11 same-class no-explicit-any in the new repo + test fake, the T-093/T-080 pattern).
...
**T-215 (hub, TESTED + live-verified):** NEW `scripts/policy_census.sh` — the REG-004-class hardening the 31st session recommended: machine-checks the LIVE pg_policy set (pg_policies, schemaname='public') against the canonical chain's cumulative CREATE/DROP POLICY statements (drops + DROP TABLE cascades applied in numeric order; storage-schema policies out of scope; LAST creator wins) at session openings. Live run at T-215: **chain=189 live=189, zero drift** (live-only would be the REG-004 class, chain-only the ARCH-011 class). Parser pinned by `src/tests/infrastructure/t-215-policy-census.test.ts` (5/5, real-script invocations incl. throwaway dirs via --dir). Desktop suite baseline re-verified green with migration 0080 in the chain (92 files / 2422 / 0 + typecheck).

**T-217 (desktop, TESTED):** T-047 port #6 — NEW SupabaseWorkforceAttendanceRepository over `workforce_attendance_events` (0010): the worker dashboard's clock punches, the manager dashboard's daily feed and the employee drawer's attendance history now persist (mock slot was memory-only). `latestFor` promoted into the workforce AttendanceRepository domain interface (it was mock-only surface the worker dashboard's clock-state memo consumes synchronously — both implementations answer from their local cache now; the provider slot type changed from `typeof mock…` to the aliased interface). DISCOVERY persisted in-repo: THREE interfaces named `AttendanceRepository` exist (workforce / core / academic — student attendance) — the provider import must alias or the slot silently gets the wrong contract. Punch instant is server-side (event_at default; the input date is advisory); metadata.ip dropped (no column — documented divergence); tests 12/12; suite 94 files / 2439 / 0 + typecheck clean.

**T-216 (hub, VERIFIED):** the closing MIG-TOKENS round for the owner's mandate — 10/10 probe families GREEN (chain **77/77 = 0001–0080 zero drift** incl. this session's 0080; policy census 189/189 zero drift — T-215's script is now part of the closing ritual; dual-key health ADR-009; EF fleet 13/13 ACTIVE 1:1 with hub source; ALLOWED_ORIGINS canonical 4-origin set echoing, non-allowlisted NOT echoing; anonymous EF 401; 0080 live). One probe lesson persisted: verify the canonical origin string before suspecting an allowlist regression (the first probe used `elimtiyaz.vercel.app`, the real production origin is `elimtiyaz-website.vercel.app`). Evidence: `docs/recovery/t-216-live-verification.md`.

**T-218 (hub, closeout):** 32nd-session close — registries truth-synced (task registry T-209..T-218 all Completed with evidence; problem registry 199 detailed entries [+INFO-300 TESTED]; current-state 32nd-session snapshot; next-task CLOSED + the next-session recommendation), all three repos pushed with the owner's PAT + zipped. Suites at close: website 40 files / 544 / 0 + lint + strict build; desktop 94 files / 2439 / 0 + typecheck + lint 0 err; Android untouched (zero commits — no Android-scope task this session).

## 33rd repair session (2026-09-07) — owner mandate: payment-modal 16:9 fix (no cut-off) + payments-journal issuer/timestamp + FULL DAG automations

**T-219 (desktop, TESTED):** UI-305 closed at the design-system level — UnifiedModal dialog variants were `grid … flex-col` with NO height cap (the body's `flex-1` was inert; tall forms pushed the footer off-screen; the drawer variant was already correct). Dialogs are now flex columns capped at `max-h-[88vh]`; NEW `2xl` wide-form tier (max-w-6xl ≈ 1152px ≈ 16:9 stage) registered in the size maps; the UnifiedPaymentModal moved to 2xl and restructured into a responsive 12-col two-column form (7: identification/summary/slider/waterfall preview; 5: method/structured check-wire fields/proof/debt meter/notes/status) with the payer+amount recap pinned in the always-visible footer; the success stage is a two-column receipt summary (issuer, collector, exact date & time). Regression source-guards shipped (2xl registration, height cap present, old grid layout FORBIDDEN, two-column split pinned).

**T-220 (desktop, TESTED):** the payments journal now identifies WHO issued each payment and WHEN: the Émetteur (Payeur) column (parent avatar + full name + family code + linked student, resolved from the parents/students observables), the receipt demoted to a secondary line under the collector attribution (Encaissé par), and the exact transaction timestamp (dd/MM/yyyy HH:mm + relative secondary line). Search spans payer/student/family-code/receipt/method/category.

**T-221 (desktop, TESTED):** DAG-100 closed — the DAG builder is now a functional automation system. Node registry 17 → 29 subtypes (educational set: grade_below_threshold, payment_cleared_or_bounced, document_expiration, calendar_cron_event, stock_level_critical, time_window, route_switch, send_whatsapp, restrict_account, dispatch_task, generate_document, account_adjustment — every subtype carries label + description + palette grouping, guarded by the completeness test). NEW NodeInspectorDrawer (double-click / ⋯-menu): per-subtype parameter forms + the VISUAL PREDICATE BUILDER (Champ/Opérateur/Valeur rows × ET/OU/NON compiled into the canonical ConditionNode tree via the existing §10.05 evaluator) + switch-route editor mapped to outgoing edges + test-payload preview. NEW PURE dry-run engine (`domain/calc/workflow/dry-run.ts`): topological, branch-aware (failing condition closes only its own branch; route_switch opens only the first passing route; convergence executes once; missing fields → false + warning, never throws) — wired to the canvas "Tester" button (green taken-path edges + per-node rings + skipped fade + warnings banner) AND to the upgraded mock executor (the linear single-flag walk is gone; runs recorded in topological order with per-branch skip reasons; durations + 90% action-failure + daily cap + audit preserved). NEW canvas UX: zoom (buttons + Ctrl-wheel anchored at cursor), pan (drag background), minimap (viewport rect + click-to-jump), snap-to-grid, topological auto-layout ("Réorganiser" — grid-multiple gaps so snapping stays even). NEW 3 one-click educational templates (Pack Relance Impayés Échelonné / Alerte Assiduité & Retards / Clôture Trimestrielle) wired into the Nouveau workflow picker, each acyclic by construction with REAL condition trees, guarded by templateIsValid + collision-proof instantiation. **Residual (next backend task):** the `workflow-execute` EF still runs server-side semantics — the branch-aware engine must be ported there; action executors remain simulated; RESEND_API_KEY / FIREBASE_SERVICE_ACCOUNT_JSON stay owner-gated.

**T-222 (hub, closeout):** 33rd-session close — task registry T-219..T-222 + T-218's missed status flip truth-synced (the 32nd closeout commit had left it In Progress), problem registry +UI-305 +DAG-100 (201 detailed entries, 155 TESTED), current-state 33rd snapshot, next-task 34th-session recommendation, commits per task, zips, push with the owner's PAT.

- Suites at close: **desktop 100 files / 2513 tests / 0 failures** (was 94/2439; +6 files/+74 tests: workflow-dry-run 9, workflow-auto-layout 8, workflow-templates 11, workflow-subtype-registry 7, workflow-executor-branches 4, source guards 13) + `tsc --noEmit` clean + eslint **0 errors** (401 warnings — NET −3 vs HEAD's 404; all new files lint-clean). Website + Android untouched (desktop-scope mandate). Live chain UNTOUCHED: still 77/77 = 0001–0080 zero drift (no migration this session — none needed).
- New problems registered (2, both TESTED): UI-305, DAG-100 — totals 201 detailed entries.
- Owner residuals after this session: unchanged (FIREBASE_SERVICE_ACCOUNT_JSON, RESEND_API_KEY, web-push env vars, AUTH-200's Google OAuth client, UNKNOWN-020 alert volume).

---

## 34th repair session (2026-09-07) — owner mandate: "Fully solve the DAG automation system — complete, production-ready automation pipeline from the visual builder through persistence and execution" — CLOSED

**Task set T-223..T-232 (10 tasks, DAG-only scope).** The session's opening audit found the live execution path DEAD (DAG-101: the deployed EF selected nonexistent columns, inserted four nonexistent ones, and dispatched by flat legacy type strings — 0 workflow_runs rows had ever been written) and the DAG-100 residuals exactly where the 33rd session left them. The ten tasks closed the ENTIRE pipeline:

- **T-223** — migration **0081** (+ live apply 78/78): workflow_runs execution-contract columns (actor_note/request_id/workflow_version/resumed_at), trigger_type CHECK extended to the T-221 subtypes, workflows.version + bump-on-publish, workflow_pending_resumes (persistent delay queue, unique pending per run+node), `validate_workflow_dag` SQL function (Kahn in plpgsql with flat edge arrays, 29-subtype whitelist, strict condition/route/wait rules) + the **workflows_publish_gate** (an invalid/cyclic DAG can never be published through ANY writer — UPDATE or INSERT). Live verify 25/25 (`scripts/verify_t-223.sql`).
- **T-224** — the **pure server engine** (`workflow-execute/engine.ts`, ZERO imports → vitest-importable): validation mirroring the SQL gate, the condition evaluator ported verbatim (§10.05 missing-field→false+warning), branch-aware topological execution (per-branch closure, route_switch first-passing, convergence-once, per-branch failure closure, deadline, max-nodes, park/resume), injectable ActionHandler. **27 tests** including node-per-node EQUIVALENCE with the desktop dry-run engine. Found+fixed the time_window context-instant bug the equivalence tests exist to catch.
- **T-225** — the **EF rewrite + deploy** (v22→v27): reads dag_definition/version (the REAL columns), writes the REAL run columns, type+subtype dispatch through the 29-subtype registry, progressive node_results persistence, park-on-long-delay, EF-owned last_executed_at/total_executions, dry_run mode, trigger-type enum validation. **Live 33/33** (`scripts/t225-live-matrix.sh`).
- **T-226** — **real action executors**: parent in-app notifications through the CANONICAL 0077 RPC (caller-scoped client — the service-role "no tenant context" bug found+fixed live), staff broadcast (in-app rows + FCM with per-recipient honest failures), REAL tasks insert, REAL account restriction + mutation audit, honest wa.me preparation, extract_field; apply_discount/create_invoice/generate_document/account_adjustment/database_query honest SKIPS (no fake writes). **Live 15/15** (`scripts/t226-action-matrix.sh`).
- **T-227** — **real execution context**: the canonical compute_parent_summary RPC, oldest-unpaid-installment days_overdue, latest payment, unexcused-absence counts; client-supplied payloads merge UNDER server values (a forged context cannot fake entity data — proven). **Live 10/10** (`scripts/t227-context-matrix.sh`).
- **T-228** — **persistent delay/resume**: the workflow-resume-scheduler EF (cron every 10 min, CRON_SECRET/service-role auth, ATOMIC claim → duplicate protection, re-entry through the same engine, honest cancellations) + migration **0082** (the insert-published gate gap found by live testing: direct INSERTs bypassed 0081's UPDATE-only gate; chain 79/79). Live multi-hop cycles: park → resume → re-park → complete; an immediate duplicate invocation claims 0.
- **T-229** — the **EF dry-run** dedicated verification: all actions simulated, zero side effects (before/after row counts), no run row, one audit entry, **prediction parity** (dry-run ≡ real execution node-per-node). **Live 10/10** (`scripts/t229-dryrun-matrix.sh`).
- **T-230** — **desktop wiring**: the Exécuter button (canonical EF path, published-only), the Test serveur mode (entity picker → EF dry-run → canvas visualization with honest failure rings — DryRunNodeResult gains the documented 'failed' status), the domain dryRun contract + mock/supabase implementations. 8 new tests.
- **T-231** — **Android contract fix**: WorkflowRunDto now carries the REAL columns (trigger_type/actor_id/completed_at/duration_ms/node_results + the workflows(name) embed via Columns.raw), node_results decode into the domain model (no Room schema change), WorkflowTrigger.fromCode maps the real codes (unknown → Event, never silently "Manuel"). **Gradle 49 suites / 416 tests / 0 failures** (JDK 21 + SDK 35 re-provisioned in-session).
- **T-232** — the **final e2e matrix through the DESKTOP's exact PostgREST+RLS contract**: create (RLS insert) → cyclic publish REJECTED → publish (version 1) → lossless reload → trigger → real-data conditions (the 1.5M DZD debtor) → route divergence → real actions (honest undeliverable for the unlinked parent, real delivery to the linked one) → run row + audit → execution-history queries → park → unpublish → honest cancellation. **Live 18/18** (`scripts/t232-final-matrix.sh`; full record: `docs/recovery/t-232-live-verification.md`).

**Suites at close:** desktop **105 files / 2578 tests / 0 failures** + tsc clean + eslint 0 errors; Android **49 suites / 416 tests / 0 failures**; website untouched (no DAG surface by design). **Registry:** DAG-100 → **VERIFIED**; DAG-101 → RESOLVED (T-223/T-225); migrations **0001–0082 zero drift**; EF fleet 14/14 ACTIVE (workflow-execute v27 + workflow-resume-scheduler v1 new).

**Owner-gated residuals (honest, documented):** RESEND_API_KEY (email delivery), FIREBASE_SERVICE_ACCOUNT_JSON + FIREBASE_PROJECT_ID (FCM push), WhatsApp Business API (wa.me link preparation only), canonical financial RPCs for apply_discount/create_invoice/generate_document/account_adjustment (honest skips until the business rules are ratified).

---

## 35th repair session (2026-09-07) — owner mandate: Personnel/Workforce RBAC overhaul — strict role workspaces (desktop + Android), teacher self-containment, data-layer scoping, operational-dashboards persistence — CLOSED

**Task set T-233..T-242 (10 tasks).** The session opened with the owner's rejection of an external reviewer's assessment ("wrong obviously") and the mandate for a production-grade overhaul. The opening ritual verified the review line-by-line: four client-side claims CONFIRMED, two REBUTTED (account provisioning IS built — T-079 EF, live-proven this session; most "mock-only" matrix rows stale — T-099..T-217 already ported them), and TWO NEW data-layer findings the review missed entirely (0019 RLS grants teachers students UPDATE — DB-level student-profile writes — plus tenant-wide students/parents SELECT). The session then closed every layer of the boundary:

- **T-233** — TEST-306 flaky fixture fixed first (baseline purity): `att-early` re-stamped to same-UTC-day midnight (always ≤ the punch instant); commit 608a500. Suite green at every hour afterwards.
- **T-234** — **RBAC-300 desktop half**: permissions.ts matrix — Teacher LOSES ViewRoster+ViewAcademics (CRM + Pédagogie sidebar → padlock; action permissions retained for in-Personnel use), SupportStaff GAINS ViewFinancials+ManageClasses+PromoteStudent (clerk front-office: payment logs, class assignment, year-end promotions), Manager loses ViewFinancials (0019 payments_select server parity); app-shell.tsx route-guard table on /crm, /academics*, /financials (URL bypass closed, defense-in-depth); RED-first role-boundaries suite pinning every role × module; commit 1a10a60.
- **T-235** — **RBAC-301 teacher workspace self-containment**: RollCallScreen + GradeEntryScreen composed as optional-prop overlays INSIDE the TeacherDashboard (routes still work standalone); every `navigate('/academics/...')` removed; source-guard test pins zero /academics navigation in the teacher dashboard; commit 7306749.
- **T-236** — **RBAC-302 data-layer fix**: migration **0083** (append-only, idempotent, applied live atomically — HTTP 201, MIG-TOKENS pattern): students_update drops 'teacher' (teachers can NEVER write student profiles — matches the owner's spec), students_select teacher branch scoped to classes-they-teach (homeroom via classes.homeroom_teacher_id OR subject via class_subjects.teacher_id), parents_select drops 'teacher' (no parent-facing duty), assessments_select scoped the same way; all other roles byte-identical; commit 0321644.
- **T-237** — **Android mirror (cross-platform §10)**: Rbac.kt matrix lockstep (teacher loses VIEW_ROSTER/VIEW_ACADEMICS; clerk gains MANAGE_CLASSES/PROMOTE_STUDENT; manager loses VIEW_FINANCIALS; financial_officer gains VIEW_ACADEMICS to match the 0023 server seed — Android was the lone divergent platform); NEW "Mon espace" first tab in PersonnelHubScreen (TeacherWorkspaceScreen + ViewModel: STRICTLY homeroom-scoped class list — unlinked teacher sees ZERO classes, never the catalog — with Appel/Notes routing to the ROLL_CALL/ENTER_GRADES-gated routes); name-based tab selection (robust to the conditional layout); deep-link degradation safe; 6 new RolePermissionTest boundary tests + 5 TeacherWorkspaceTest scoping tests; android commit 655b404.
- **T-238/T-239/T-240** — **ARCH-001 slots #7/#8/#9 closed** (the operational dashboards' production-grade persistence — the "no mock workarounds" mandate): SupabasePurchaseRequestRepository (PR-YYYY-NNNN deterministic numbers, 23505 retry recount, frozen requester/approver display names per the 0074 precedent, one-update state transitions mirroring the 0011 CHECK), SupabaseDeliveryRepository (DLV-YYYY-NNNN, lifecycle timestamps, guarded delay path through reportDelay, frozen driver name), SupabaseInventoryRepository (soft delete, scan flow, insert-only audit ledger with frozen before/after + actor) — onto the canonical 0011 tables; migration **0084** (frozen display/audit columns, applied live atomically + post-checks) via scripts/apply_0084_live.sh; 25 new tests; commits fc11d3f / 3954a1b / e79ac93.
- **T-241** — **the live verification round**: `scripts/t241-live-matrix.sh` — 14/14 GREEN through the desktop's exact PostgREST+RLS path: provisioning round-trip (super_admin → EF → teacher AND driver accounts created, both sign in immediately, profiles active + roles assigned + audit entries — the owner's "admins can reliably create user accounts" requirement PROVEN), teacher scoping (students 0 / parents 0 / payments 0; students UPDATE 0 rows affected with a succeeding admin control on the SAME row — the deny is role-specific), escalation denial (teacher → EF → 403), operational lockout (driver: 0/0); evidence: `docs/recovery/t-241-live-verification.md`; commit 1e3f6eb. PGRST204 lesson persisted: probe UPDATEs must target REAL columns.
- **T-242** — this closeout: problem-registry 35th-session section (TEST-306 FIXED, RBAC-300/302 VERIFIED, RBAC-301 TESTED; 205 detailed entries), ARCH-001 slot-closure note, task-registry T-233..T-242 Completed-with-evidence, this change-log section, current-state 35th snapshot, next-task 36th recommendation, zips + push.

**Suites at close:** desktop **112 files / 2687 tests / 0 failures** (was 105/2578; +7 files / +109 tests: purchase-request 9, delivery 9, inventory 7, role-boundaries + route guards + teacher-workspace from T-234/T-235, +60 from the 34th session's baseline) + tsc clean + eslint 0 errors on new files; Android **50 suites / 427 tests / 0 failures** (was 49/416; +TeacherWorkspaceTest 5, +RolePermissionTest 6 boundary tests); website untouched (parent/student portal — separate RLS policies, verified unaffected by 0083). **Live chain: 0001–0084 zero drift** (0083 + 0084 both applied atomically this session with post-checks).

**Registry:** RBAC-300 → **VERIFIED** (desktop + Android + server + live probes); RBAC-302 → **VERIFIED** (0083 live + T-241 matrix); RBAC-301 → TESTED (suite-pinned); TEST-306 → FIXED; ARCH-001 → 3 more slots closed (16 of the original 25 now ported; remaining: releve, shifts, schedules, performanceReviews, onboarding, warehouseTasks, pricing, aiConfig, backups, clubs/psychology/orthophonie/teachers).

**The external review, adjudicated (owner's rejection sustained):** ~70% accurate. Confirmed: the four client boundary gaps. Missed: the two Critical server-side RLS grants (fixed by T-236), the fact that account provisioning was already built and live-provable (T-241 proved it), and the stale mock-matrix rows (attendance/leave/tasks/suppliers/chat/calendar/workflows were already Supabase-backed since T-099..T-217).

**Owner-gated residuals (honest, documented):** unchanged from the 34th session (RESEND_API_KEY, FIREBASE_SERVICE_ACCOUNT_JSON, WhatsApp Business API, canonical financial RPCs for the DAG action set) + the admin password re-set during T-241 should be rotated by the owner at next sign-in.

---

### 2026-09-09 — 36th session (T-243..T-245) — the owner's AI-review 3-zone dashboard integration (data-honest adaptation) + live migration-token re-verification

- **Problem IDs:** UI-306 (NEW this session, TESTED) — no live drift discovered (18/18 probes green; chain 0001–0084 zero drift, EF fleet 14/14 ACTIVE, ALLOWED_ORIGINS canonical set intact)
- **Mandate:** owner's pasted AI-review integration blueprint (unified 3-zone dashboard: widget matrix + design tokens + production components for `overview-tab.tsx`) + "apply the migration tokens; ensure everything works correctly across all platforms and the migration is properly applied and consistent everywhere; zip all systems + the 3 repos, push with the PAT".
- **What changed (hub repo — desktop UI only, zero schema/API touch per §15.9):**
  - T-243 / UI-306: NEW `src/shared/ui/dashboard-theme.ts` (single Recharts styling source — gridlines/axis ticks/tooltip chrome/palette from `--brand-*`+`--status-*` tokens); NEW `src/features/dashboard/components/` with 4 components (sparkline-kpi-card, recovery-funnel-card + `deriveRecoveryFunnel`, weekly-operating-rhythm + `deriveWeeklyRhythm`, insights-rail); REWRITTEN `tabs/overview-tab.tsx` (the review's 12-col Zone A/B assembly); `dashboard-page.tsx` gained ONE payments-observable subscription passed as a separate prop (stream updates never re-trigger the aggregate fetch); `topbar.tsx` search placeholder now hints NL queries; superseded `src/tests/ui/dashboard-restructure.test.tsx` (T-088 layout guards) REPLACED by `src/tests/ui/dashboard-3zone.test.tsx` (the surviving T-088 invariants re-pinned + the new derivations/render contract).
  - Data-honesty decisions (the review's sample data vs §15.16 — each documented in UI-306): sparkline/delta OPTIONAL (only the revenue card carries them — the only REAL series in the contract); the funnel fed by REAL debtAging family counts, NOT the review's admissions 480/395/310/245 (no repository contract carries demande/dossier counts); the weekly rhythm computed from the REAL canonical Payment[] stream, NOT the review's WEEK_DATA literals; the rail's gauge = the REAL collection rate, NOT the review's 84.9M/115M pair; the AI card derives from the REAL overdue census, NOT the review's "+4.2% vs N-1" projection or "Tranche 2 (15 Déc)" deadline.
  - T-088 supersession (deliberate, owner mandate): the review's Primary Trend Card (revenue spline) returns to the Overview as the TREND view while the drill-down keeps the DETAIL view (bar + breakdown + departments pie); demographics/debt-aging-detail/tables stay drill-down-only (the review's own §5 placement).
  - T-244: live migration-token verification round (18/18 GREEN) — dual-key auth health (legacy anon JWT + sb_publishable), RLS anon 0-rows on parents/students/payments/ledger_entries/installments, chain drift check live=81/local=81 ZERO DRIFT, EF census 14/14 ACTIVE 1:1 with the hub source, anonymous EF → 401, ALLOWED_ORIGINS canonical 4-origin set (5173/3000/3100/elimtiyaz-website.vercel.app) echoing + non-allowlisted rejected, committed-key consistency vs the owner's re-supplied sheet ×3 (website publishable key, Android .env.example URL, website URL). Script persisted at `/home/z/my-project/scripts/verify_t-244_mig_tokens.sh` (outside the repos — carries the sbp_ access token).
  - T-245: registries (task-registry 36th-session summary + status flips, problem-registry +UI-306 [206 detailed entries]), this change-log entry, current-state snapshot, next-task 37th-session recommendation, zips + push.
- **Why:** the owner supplied the review as the design basis; AGENTS.md §15.16/§16 forbade rendering the review's synthesized sample numbers — so every widget was adapted to the REAL repository contract, with each deviation documented in the problem entry.
- **Affected components:** desktop dashboard feature only (overview-tab, 4 new components, dashboard-page payments wiring, topbar placeholder); no migration, no EF, no Android/website code.
- **Tests:** `npx vitest run src/tests/ui/dashboard-3zone.test.tsx` → 17/17; full `npm test` → 112 files / 2694 / 0 (5 skipped); `npm run typecheck` clean; `npm run lint` → 0 errors / 399 warnings (net −14 vs the 35th close); `npm run build` (vite production) green; website re-verified untouched-green (40 files / 544 / 0 + lint + strict `next build`); Android untouched (zero contract impact — desktop-scope UI change, same posture as the 33rd desktop-scope session).
- **Verification:** live round 18/18 GREEN (see T-244 above — the chain is UNTOUCHED this session, all checks are read-only probes).
- **Notes / residuals:** the admissions-funnel data source does not exist in the backend (a future admissions pipeline can re-point the generic funnel); only the revenue KPI carries a sparkline (other metrics need repository-level history, not client synthesis); parents/staff/pendingExpenses KPI tiles were replaced by the review's 4-slot top row (reachable via Voir détails + alerts tab) — a compact signals strip is a small follow-up if the owner wants them back. Push unblocked same-day follow-up: the owner re-supplied a valid PAT → e9cedfd + 3858e39 landed on origin/main (c74c7cc..3858e39); android/website fetched and confirmed in-sync (zero delta); task-registry T-245 status line truth-synced by the follow-up commit.
