...
- **Status:** CLOSED — TESTED (2026-09-13): full desktop suite 140 files / 3 127 tests / 0 failures + `tsc` 0 errors + eslint 0 errors; the real-workbook import replays every row (C3 totals EXACT: 113 518 800 == 113 518 800) with the BON 4-tranche shape per student. Migration 0090 committed; the LIVE apply is prepared (`scripts/apply_0090_live.sh`, T-091 atomic pattern) but awaits the owner's sbp_ token (never persisted, per token hygiene) — until applied, a live Supabase import must NOT run (the 0007 CHECK would reject tranche 4).
