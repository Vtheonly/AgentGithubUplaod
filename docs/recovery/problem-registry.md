...
- **Proposed resolution:** IN PROGRESS — T-223 (migration 0081, DONE + live-verified) aligned the schema; T-224/T-225 rewrite the EF on a pure, unit-tested engine reading `dag_definition` and dispatching by type+subtype. The verify evidence: `scripts/verify_t-223.sql` 25/25 GREEN post-apply (live), plus the desktop source-guard `workflow-dag-server-validation.test.ts` (10 tests) pinning the SQL whitelists to the TS registry.
