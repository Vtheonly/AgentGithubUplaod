...
## T-412 — Personnel Payroll Cash-Flow Forecast & Pre-Payroll Funding Requirements — IMPLEMENTED / TESTED (P0)
...
**Status (96th session, 2026-09-25 — EXECUTED in 6 commits):** IMPLEMENTED / TESTED — `642a3e0` (Phase 0: the registry + ADR-024) → `53eb449` (Phase 1: the canonical engine `src/domain/calc/payroll/payroll-forecast.ts` + 22 unit tests incl. the owner's 30×1M DZD = 30M example) → `1c22404` (Phase 2: the Personnel « Paiements du Personnel à Venir » section + 6 UI tests) → `ad49ceb` (Phase 3: the Finance pre-payroll funding card + the optional treasury payroll block + 10 parity tests) → `7c9f5ec` (Phase 4: the Statistics trend — `derivePayrollCostTrend` + the monthly/quarterly card + 5 more parity tests + 4 UI tests) → the Phase 5 docs close-out (this commit). Gates: tsc **0 errors** (the baseline was repaired to 0 by the concurrent agent's 05766c9 mid-session) / eslint 0 errors on every changed file / the FULL vitest failing set **byte-identical to the pre-change baseline** (25 failures — the pre-existing dashboard/cross-platform/t-034/t-390/vault/t-134 families from the owner's `oaky` commit and earlier; diffed FAIL-line by FAIL-line) / +**47 new T-412 tests, all green** (22 engine + 6 Personnel UI + 15 parity + 4 Statistics UI). WORKFORCE-504 flipped RESOLVED / TESTED. **Remaining for VERIFIED:** the owner's packaged-app UI pass over the three new surfaces (the Personnel forecast section, the Finance funding card, the Statistics trend card) — the same convention as T-408/T-409/T-410/T-411. NO backend migration was needed (read-side feature by ADR-024 design) → no live-verification doc; the evidence lives in this registry + the change-log + the parity suite.
