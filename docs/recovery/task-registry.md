...
## Progress summary (2026-09-02, updated at the 19th repair session CLOSE — owner mandate "fix the auth thing + apply the migration tokens, consistent everywhere": T-115/T-118/T-116/T-117/ARCH-012 COMPLETE + T-107 follow-through; 19th-session opening check found live drift (0065, ARCH-013) — CLOSED with the migration reconstructed + committed + verified 19/19; chain now 62/62 = 0001–0065 zero drift; suites: desktop 2236, website 429, Android debug 331 + release 326 (first time BOTH variants green) — all green)

| Status | Count | Tasks |
|---|---|---|
| **Completed (VERIFIED)** | 8 | T-000, T-079, T-004, T-094 (live integration suite 5/5, 2026-08-31), **T-068** (live deploy + curl matrix + permission probes, 11th session), **T-095** (live 200 + idempotency, 12th session), **T-103** (15th session — finance reconciliation + read consistency, live 8/8) |
| **Completed (TESTED)** | 64 | T-001, T-003, T-009, T-078, T-081, T-019, T-049, T-002, T-065, T-016, T-027, T-061, T-031, T-029, T-071, T-083, T-084, T-088, T-080, T-089, T-091, T-087, T-092 (sessions 1–9) + T-006, T-008, T-093, T-032, T-035, T-056 (10th session) + **T-011, T-012, T-013, T-014, T-023, T-025 (migration 0057 live 6/6), T-033, T-048, T-060** (11th session) + **T-015 (0058 live 7/7), T-053, T-022, T-040, T-052, T-057, T-055, T-018 (desktop+sync)** (12th session) + **T-041 (migration 0059 live 10/10; desktop 2146 tests), T-030 (migration 0060 live 9/9; website 135/135)** (13th session) + **T-058 (append-only migration guard: 9/9 matrix + 6/6 suite tests)** (13th session) + **T-050 (OnlineDetector fail-closed + own-backend probe + pullAll dedup; desktop 13/13 new tests, Android 15/15 new tests)** (13th session) + **T-036 PUSH-103 portion (website FCM auto-registration after first user gesture; 9/9 new tests, suite 144/144)** (13th session) + **T-026 (overdue rule aligned: 10/10 new tests, Android suite 275/275)** (13th session) + **T-054 (hollow implementations real: 7/7 new tests)** (13th session) + **T-062 (dead code removed: 5/5 new tests; debug APK 29.8 MB)** (13th session) + **T-063 (absence-alert threshold ≥3/current-term: 10/10 new tests)** (13th session) + **T-064 (config-dialog security + placeholder detection: 9/9 new tests)** (13th session) + **14th session: T-096 (portal out-of-the-box config, website 153/153 + live render), T-097 (Electron ESM start fix, launch-verified under Xvfb), T-098 (migration 0061 live 15/15), T-099 (SupabaseChatRepository 12/12), T-100 (staff↔parent channel entry), T-101 (website chat readiness 4/4), T-102 (Android chat gap registered)** + **17th session: T-020, T-021, T-046, T-051, T-017-interim, T-106, T-107, T-104, T-034, T-108** + **18th session: T-024 (Android homework UUID + promotion, live 5/5), T-039 (pull completeness, 16 new tests), T-046-gap (Room schemas 12+13.json + MigrationTestHelper 4/4), DTO-TENANT (null-tenant pull default, 7 tests), T-082 (lint gate restored — desugaring + baseline, 0 errors), WEAK-030 (migration 0064 live 11/11), T-107-residual (publishable-default render verified), WEBSITE-DOC (AGENTS.md realtime claims corrected)** + **19th session: T-118 (DRIFT-001 closed — mock + approve EF deterministic, EF deployed live), T-116 (AUTH-200 portal UX mitigated, provider still owner-gated), T-117 (registry hygiene 12 headers + t-052 portability), ARCH-012 (release variant green, 2 documented exclusions), T-107-follow-through (credentials sheet + §7 checklist re-run)** |
| **Completed (IMPLEMENTED)** | 1 | T-010 (launch verification needs a desktop host) |
| **In Progress (20th session close)** | 1 | **T-102-follow-up** (Android chat read-side + online sends) — DEFERRED AGAIN (4th deferral; a full feature build needing a dedicated session; T-069's channel infrastructure is NOW LANDED, so the next session can build the chat read-side on it directly — the routing map entry for `chat_messages` is a one-line addition). The Android toolchain is re-provisioned and green (JDK 21 + SDK 35 + the .env secrets-plugin quirk — recipe in scripts/android-env.sh) |
| **Ready** | 2 | T-043 (equivalence consolidation, ADR-006 — needs a full dedicated session), T-044 (Android design-system consolidation) — 20th-session hygiene: T-024/T-039 were completed in the 18th session but still listed here (stale row fixed); T-069 completed this session (TESTED) |
...
### T-125 — Registry truth-sync (21st session) — **In Progress**
...
### T-126 — PUSH-100 substantial close: fix + canonicalize + wire the send-push-notification EF — **In Progress**

- **What:** (a) WEAK-014: `.eq("user_profile_id", …)` → `user_id` (×2, lines 197/211); (b) WEAK-015: PEM parser must strip the BEGIN header before atob; (c) propagate `priority`+`type` into the FCM `data` field (PUSH-101 part 1); (d) move the source to the hub canonical `elimtiyaz-desktop/supabase/functions/send-push-notification/` + remove the website's drifted copy; (e) replace the workflow-execute `push_notification` STUB with a real invoke; (f) deploy both EFs live + 401 curl matrix; (g) document the owner residual (Firebase service-account secret).

### T-127 — PUSH-101 Android half: FCM receiver + deep links — **In Progress**

- **What:** ElImtiyazMessagingService reads `priority`/`type` from the notification payload (with data fallback); AndroidManifest gains the click_action intent filter; unit tests.

### T-128 — CROSS-103: Android refund installment sync pushes — **In Progress**

- **What:** after the local waterfall revert in `LocalPaymentRepository.refund`, enqueue `installment` sync entities (the batch-registration pattern; dispatcher pushes via the idempotent `upsert_installment_from_import` RPC — no server-side auto-revert exists to conflict with, verified against the migration chain). Also fix the stale comment at LocalRepositories.kt:1015 ("installment is not in the SyncQueueDispatcher's switch statement yet" — false since TIER 4 / migration 0037).
