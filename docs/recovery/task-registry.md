...
**Status:** TESTED (2026-09-22 — all six stages landed: migrations 0109/0110 live-verified 27/27; canonical TS domain + native solver 25/25; TimetableRepository Supabase+mock 9/9; the dedicated Emploi du temps UI 7/7; packaging gates PASS incl. the Windows x64 portable .exe with the solver verified inside the packaged ASAR + clean-environment generation under `env -i`. Full evidence: docs/recovery/t-404-live-verification.md. Remaining to VERIFIED: executing the built .exe on a real Windows machine (owner action, same residual as T-506) + the NSIS setup variant on a wine-equipped host.) **SCHED-103 follow-up (same day, live-reported):** the shipped problem assembly 400'd live (PGRST200 — class_subjects→personnel had NO FK; the `personnel!left` embed cannot resolve without one). App fixed (embed removed, names resolved from the existing personnel fetch; live-verified 10/10 by `scripts/t404-postgrest-smoke.sh`; .exe rebuilt SHA256 `9330a6a9…`); canonical FK migration **0113** (drafted 0112, renumbered at the T-407 merge — the parallel session's register_family_batch 0112 applied+registered first) + `apply_0113_live.sh` ready — apply owner-gated on the sbp_ token (runbook: docs/operations/credentials.md + SCHED-103 in the problem registry).  
**Priority:** P0  
...
**Status:** TESTED (2026-09-22 — 89th session delivered the creation-integrity core (ACAD-506/507/508 + SCHED-105/106, live legs E2E-proven: the class dialog resolves the REAL academic_levels uuid via `repos.academicLevels.getByGradeCode`; the teacher model resolved to the PERSONNEL-role projection (SCHED-100/ADR-020 — NO teachers table; SupabaseTeacherRepository bridges personnel + class_subjects + the published canonical timetable); migration 0114 seeds the Algerian national catalog (14 identities + 127 configurations, the OFFICIAL BEM scale pinned) with NO second module/matière model; verify_t-408 GREEN; the FK smoke 10/10; E2E class + subject creation 201 zero-residue; evidence: docs/recovery/t-408-live-verification.md). **The 90th session completed the E/F workstreams**: ACAD-509 RESOLVED — the synthetic academic-YEAR layer purged (the hook's fabricated "ay-2025-2026" fallback pair REMOVED for the honest null contract; every year-scoped creation surface guards the missing-year case with a clean error; the stale "2025-2026" literals purged from the class dialog, the grade-entry payload (a PERSISTED column), mapSubjectRow/mapClassRow/homework; the year drawer derives from subject_configurations; the dashboard's hardcoded four-year selector now derives from canonical academic_years) — pinned by 8 source guards + the live probe scripts/t-409-live-year-payload-probe.py 8/8 GREEN (201 with real FKs, reload-safe, the synthetic id 400/22P02, zero residue); the module/subject/matière census RECORDED (one canonical Subject concept confirmed across Desktop/Supabase/Android/Website + every named surface — no parallel model exists); the provider-wiring census confirms every academic slot overridden in Supabase mode (mock reachable only behind VITE_USE_SUPABASE=false). Gates: FULL 3749/21-baseline + tsc 0 + eslint 0; website 648/648 + tsc 0 + build. **VERIFIED remains gated on:** the Android equivalence run (JDK toolchain not provisioned; zero shared contracts changed by the purge) ~~+ the live teacher E2E with a REAL personnel row (owner must register staff — live personnel = 0 active)~~ — **the teacher E2E gate is CLOSED (91st session, 2026-09-22)**: the owner authorized clearly-marked FAKE test data, and `scripts/t-408-fake-academic-e2e.ts` ran the FULL chain live through the canonical repositories — 8 personnel → the staff_category flips → 2 FAKE subjects → 5 classes (REAL level/year uuids, homeroom FKs) → 52 class_subjects (118 weekly hours) → 9 rooms → 3 constraints → generation 118/118 placed with zero conflicts → review → approve → **publish (the first live `fn_timetable_publish`)** → the portal view 118 rows admin / 401 anon → the interaction suite (Matières-Classes, the teacher registry 8/8, the homeroom read-back, the T-404 manual-adjustment pin-survival contract) — **80/80 GREEN**, with the purge tool verified 5× (zero residue, catalog preserved) and five new findings registered (SCHED-107/108/109/110 + ACAD-510). The FAKE dataset PERSISTS live for the owner's own timetable UI testing (purge: `scripts/t-408-fake-data-purge.py`; convention: AGENTS.md §15.50). The real-staff swap-in remains a formality through the same canonical paths. Evidence: t-408-academic-setup-creation.md §"The 91st session" + t-408-live-verification.md §11.)
**Problem:** ACAD-505 (the umbrella registration) — closed by ACAD-506 (class uuid), ACAD-507 (phantom zod field), SCHED-105 (mock-only teachers), ACAD-508 (the missing catalog + the year-code NULL), SCHED-106 (the portal timetable), ACAD-509 (the synthetic year layer — the 90th session)  
**Priority:** P0 — Critical  
...
## T-411 — Finance UI Unification: single-source the Financials derivations, repair the consolidated-collection semantics, and reconcile the cross-tab metrics — IMPLEMENTED / TESTED (P0)

**Registered:** 2026-09-23 (the 94th session — the inspection-only Finance UI audit) · **Executed:** 2026-09-23 (95th session — Phases 0-5 complete in 6 commits + the portal commit)  
**Problem:** DUP-006, DUP-007, BUSINESS-106, BUSINESS-107, BUSINESS-108, DATA-023, DATA-024, DATA-025, DATA-026, DATA-027, DATA-028 (remise half), DATA-029, DATA-032, DATA-033, DATA-034, DATA-035, DATA-036, DATA-037 — all flipped RESOLVED/TESTED with recorded evidence  
**Related:** audit report `docs/audits/finance-ui-architecture-audit-2026-09-23.md` / ADR-023 (NEW — the cross-category collection semantics) / migration 0115 (applied live) / T-060, T-103, T-164/166/167/168, T-330, T-333, T-354, T-405 / evidence: `docs/recovery/t-411-live-verification.md`  
**Status:** IMPLEMENTED / TESTED — desktop commits 27f3083 (Phase 0 registry) → e63447d (Phase 1: ADR-023 + BUSINESS-106 + BUSINESS-107 + migration 0115, live round-trip 7/7) → d4a4a64 (Phase 2: the diagnostic engine re-base) → 4b2735e (Phase 3: tab rationalization + anomaly honesty + deep links + the INV-4 family) → 2e0f589 (Phase 4: pagination + the remise threading); website eba4606 (Phase 5: the DATA-032 B1-B9 family). Gates: desktop tsc 0 / FULL vitest 3 859 with the failing set byte-identical to the 21-test baseline / eslint 0 errors / append-only guard OK; website 648/648 + tsc 0 + lint + build green. **The VERIFIED gate:** the owner's packaged-app UI pass over the changed surfaces (the Multi-services collection mode, the Créances labels, the diagnostic numbers, the portal statement) + the Android equivalence run (ADR-023's nullable category is a shared-contract change).
...
## T-412 — Personnel Payroll Cash-Flow Forecast & Pre-Payroll Funding Requirements — VERIFIED (P0)
...
**Problem:** WORKFORCE-504 (registered BEFORE any fix per §13; renumbered same-session — the drafted WORKFORCE-501 collided with the concurrent 70th session's personnel-linkage umbrella, the §15.43-class duplicate-ID hazard) · WORKFORCE-505 + WORKFORCE-506 (the second round's findings, 98th session) · DUP-006 residual (the FA-08 payroll-in-treasury owner option — resolved by ADR-024 in the read-side direction)
**Related:** T-369 (the payroll persistence surface: salary_payments + salary_adjustments + the canonical RPCs) · T-400 (personnel E2E) · T-411 (the canonical finance architecture + the treasury "hors masse salariale" labeling) · ADR-024 · ADR-010/023 (display + collection semantics this feature must respect)
...
**Status (96th session, 2026-09-25 — EXECUTED in 6 commits):** IMPLEMENTED / TESTED — `642a3e0` (Phase 0: the registry + ADR-024) → `53eb449` (Phase 1: the canonical engine `src/domain/calc/payroll/payroll-forecast.ts` + 22 unit tests incl. the owner's 30×1M DZD = 30M example) → `1c22404` (Phase 2: the Personnel « Paiements du Personnel à Venir » section + 6 UI tests) → `ad49ceb` (Phase 3: the Finance pre-payroll funding card + the optional treasury payroll block + 10 parity tests) → `7c9f5ec` (Phase 4: the Statistics trend — `derivePayrollCostTrend` + the monthly/quarterly card + 5 more parity tests + 4 UI tests) → the Phase 5 docs close-out. Gates: tsc **0 errors** (the baseline was repaired to 0 by the concurrent agent's 05766c9 mid-session) / eslint 0 errors on every changed file / the FULL vitest failing set **byte-identical to the pre-change baseline** (25 failures — the pre-existing dashboard/cross-platform/t-034/t-390/vault/t-134 families from the owner's `oaky` commit and earlier; diffed FAIL-line by FAIL-line) / +**47 new T-412 tests, all green** (22 engine + 6 Personnel UI + 15 parity + 4 Statistics UI). WORKFORCE-504 flipped RESOLVED / TESTED. NO backend migration was needed (read-side feature by ADR-024 design) → no live-verification doc from the 96th session; the evidence lives in this registry + the change-log + the parity suite.

**Status (98th session, 2026-09-26 — the owner's issue-#11 SECOND-ROUND verification mandate: "a deeper second round of testing to make sure there are no remaining issues"):** **VERIFIED.** The deeper round (commits `33ca46c` → `c566e96` → `1f9c8ca` → the docs close-out) re-ran every first-round gate from a clean clone (all reproduced), then went deeper: a NEW 23-test white-box suite (`src/tests/domain/calc/t-412-second-round-verification.test.ts` — boundary predicates, data hygiene, the exact 30-day window edge, cross-surface restatement on roster changes) which found and fixed **two genuine defects the first round missed**: WORKFORCE-505 (the eligibility gate keyed on CURRENT-period eligibility — a future-hired roster produced NO waves) and WORKFORCE-506 (the overdue gap anchored on the last recorded period INCLUDING current rows — a current-period disbursement silently hid earlier missed payrolls; **live-proven fixed** by the E2E STEP 5). Final gates: **70/70 T-412 tests** (47 first-round unchanged + 23 second-round) · tsc 0 · eslint 0 · FULL vitest 3 962 passed / 25 failed (the failing set **byte-identical** to the documented baseline families; +23 = the new suite) · **live read-only verification 6/6 GREEN** (`scripts/t-412-live-forecast-verification.ts` — the engine over the real streams, cross-checked against independent SQL aggregation to the centime) · **live E2E 15/15 GREEN, zero residue** (`scripts/t-412-payroll-forecast-e2e.ts` — the complete loop: probe personnel → forecast → the canonical record_salary_disbursement RPC → restatement → idempotence → the 506 live proof → parity over mutated data → cleanup). financial-rules §16 amended (the per-period eligibility + the pre-current gap anchor + the interior-gap owner decision). Full record: **`docs/recovery/t-412-second-round-verification.md`**. The owner's packaged-app UI pass over the three surfaces remains the standing visual-acceptance convention (as with T-408..T-413); every machine-verifiable layer beneath it is green.


## T-413 — Student Approval, Enrollment, Portal/Messaging Synchronization & Cross-Section Navigation — IMPLEMENTED / TESTED (P0)

**Registered:** 2026-09-25 (the 97th session — the owner's T-413 mandate)  
**Problem:** STUDENT-100 (Critical), STUDENT-101 (High), STUDENT-102 (High), STUDENT-103 (Critical), STUDENT-104 (Medium) — all RESOLVED / TESTED with recorded evidence  
**Related:** T-029/T-132 (PARENT-101/102 — the guard family this extends), T-331 (the rebind-guard picker), T-264/OPS-309 (the maybeSingle convention), RLS-500 (T-384 — the SELECT-fold + zero-row lessons re-confirmed live), OPS-314 (the recursion class caught live by this task's E2E), ADR-012 (0067 — the messenger scope), migration 0116 (applied live + registered, chain head 0116)  
**Implemented:** 2026-09-25 (the 97th session — Phases 0-4 complete: the baseline repair 05766c9 → migration+EF+live-E2E → the desktop repository/UI → the Pedagogy search + the 3-dot menu → the website legs 5c530b6)

### The mandate (the owner's brief)

Fix the complete student onboarding and synchronization flow so approved students become fully active across the system — class enrollment, portal access, messaging, Pedagogy visibility/search — plus a unified 3-dot student/person menu with "Open in…" navigation preserving the canonical student identity. Verify end-to-end: application → approval → student creation/synchronization → class assignment → portal access → messaging → Pedagogy visibility/search → normal system usage. Regression tests for every leg.

### What was delivered

1. **Migration 0116** (applied live atomically with registration): the `student_application` column + the RLS-guarded self-attach (own-pending SELECT/UPDATE policies + the column-guard trigger with the service-role/DBA/staff bypasses); the `approve_student_application` composite RPC (bind-or-create student + the deterministic parent + the ELV code + class enrollment + the canonical academic-history entry + activation, REUSING `approve_account_request` internally); the `parents_student_sees_own` RLS via the recursion-safe `is_own_parent_via_student` helper; the `open_parent_admin_channel` student eligibility.
2. **approve-signup-request EF** (deployed live, smoke matrix 401/401/401/CORS-200): the STUDENT-102 guard, the parent+target_student_id category guard, the approval-time parent↔student reclassification (`assign_role` — the SEC-108 reality), the composite path (one RPC round trip).
3. **Desktop**: the repository's student matching (the previously-dead `student_match`) + `approveWithExistingStudent`/`approveWithNewStudent`; the ApprovalsTab student flows (the application-payload card, the bind-existing canonical search, the create+enroll form with the level-scoped class picker + the family resolution, the ReclassifyToggle, the validation gates); the Pedagogy « Annuaire élèves » tab (canonical search: name / ELV code / family / class / level + filters + the deep link); the shared `StudentActionsMenu` (the 3-dot "Ouvrir dans…": CRM / Pédagogie / the family dossier / the family finance / the class view — context-less actions never render) mounted on the CRM students table, the class roster and the directory; the `/financials?familyId=` deep link + the installments family filter chip.
4. **Website**: the auth-provider's student-bound resolution (self-scoped children + the family row); the `StudentApplicationForm` on the pending screen (the self-attach path, the zero-row guard, tri-lingual).
5. **Tests**: 28 (desktop approval contract) + 16 (cross-section navigation matrix) + 9 (website legs) new; the two updated source-scan guards (t-264's 7 maybeSingle sites; t-331's multi-branch submit gate).

### Evidence

- `scripts/t-413-apply.py --verify`: **19/19 GREEN** in BEGIN/ROLLBACK (incl. the in-transaction E2E round trip, the RLS-500 read-back guard, the T9 recursion regression).
- Migration applied live atomically (chain head **0116**) + re-applied the corrected body after the live-caught recursion fix; the live policy verified as the helper-based form.
- `scripts/t-413-student-e2e.py`: **20/20 GREEN, zero residue** — signup (SEC-108 parent hardcode) → self-attach (own JWT, RLS) → STUDENT-102 rejection + reclassification → the composite approval (student ELV code + parent + binding + role + activation + academic history) → the portal self-resolution (both RLS paths) → the student admin-channel open → leg B (family enrollment: parent bound + child created).
- Desktop: tsc 0 / eslint 0 errors / FULL vitest 3 939 passed with the failing set **byte-identical to the pristine-tree runs** (stash-verified on both halves — the documented 21-test baseline + the concurrent T-412 additions; zero regressions).
- Website: 657/657 + tsc 0 + lint clean + `npm run build` green (the strict gate caught the phase-narrowing issue the vitest esbuild pass had absorbed — §15.25 re-confirmed).
...
**Task 2 — Excel Import Configuration & Generic Import Engine (IMPORT-111): IMPLEMENTED / TESTED.**
- **Deep comparison FIRST** (the mandate's order): `docs/architecture/excel-format-comparison-2026-2027-vs-2027-2026.md` — the complete column mapping (identical A–E/G–Y; F `NOM`→headerless → positional; S `V2`→`V1` relabel → canonical key `v2`; ORTH/E-PLANT/Ratrapage/quarterly → PSY1–14 + CREANCE SEPT ×3 + TT CREANCE + COURS SUP/LIVRES/CLUB/SORTIES), formula equivalence (P and Q byte-identical; L same logic with year-price constants — the Task-1 per-year pricing concern; CREANCE SEPT genuinely new + informational), the relationships that must survive, and a populated-column census (the new columns are structural placeholders — the E2E uses a synthetic workbook).
- **ADR-026**: import configurations are versioned DATA documents resolved by a central registry; the engine stays format-agnostic; three-way column addressing (header / aliases / positional letter); name+header-signature format detection; ONE canonical record contract shared by all formats; financial data through the EXISTING ledger; extension points as interfaces ONLY (profile matching NOT implemented).
- **The registry** (`src/infrastructure/excel/import-config/`): `ImportConfigDocument` + `ImportConfigRegistry` (register/load/identify/select/validate/version/resolve/manage) + `compileSheetConfig` + structural validation + the `configs/` documents (etat-2026-2027 with its 4 sheets; etat-2027-2026 with the new grid + statistiques excluded) + `extensions.ts` (EntityMatcher/NoOp, ImportConfigStore, validation/reconciliation/source seams).
- **Engine integration (additive only)**: FieldSpec +`column`/`aliases`; the parser emits synthetic `__col_<LETTER>` keys + a public header-row reader; `lookupValue` resolves column-first then aliases; `processSheet` detects with the ACTUAL header row (format disambiguation); `schemas/index.ts` + the four named schemas are DERIVED from the registry (one source of truth — the legacy detection order preserved).
- **Storage adapter**: PSY3–14 → therapy_psychology; COURS SUP → tuition; LIVRES → books; CLUB/SORTIES → extracurricular — through the SAME ledger + payments + installments streams; CREANCE SEPT ×3 + TT CREANCE deliberately NOT ledgered (INV-1).
- **Tests**: +14 (7 registry + 7 new-format E2E incl. the CANONICAL EQUIVALENCE proof — the same student in both layouts imports to identical canonical records and identical ledger totals); the legacy real-excel-import suite re-ran 9/9 through the registry-backed engine (the Excel/ path candidate added so the suite runs where the workbooks live). Full suite 3 973 passed / 25 failed / 24 skipped — the failing set byte-identical to the pre-change baseline.
- **Gates**: tsc 0 · eslint 0 errors on every changed file.
...
**Status:** IMPLEMENTED / TESTED (99th session, 2026-09-26 — executed in 5 commits: `d4540e1` Phase 0 registration (renumbered from a T-414 draft that collided with the concurrent session's T-414 — the §15.54a discipline) → `95495c2` the crypto/vault + restore-hardening suites + the BKUP-502/503/504/505/506 fixes → `3d624b7` the sync/conflict matrix + the SYNC-108/109/111 fixes → `681494d` BKUP-501's SupabaseBackupRepository + its contract suite → `b437246` BKUP-507's migration 0119 + the LIVE verification script + the evidence doc). **Gates:** the five new suites **124/124 GREEN** (28 crypto/vault + 26 restore-hardening + 26 sync/conflict + 10 SupabaseBackupRepository + the neighbourhood updates) · tsc 0 errors on every T-415-touched file (the 54 reported errors are the concurrent session's pricing/import test files, pre-existing on origin/main — stash-verified) · eslint 0 errors on every changed file · the FULL vitest failing set **byte-identical to the documented 25-failure baseline** · migration 0119 applied LIVE atomically + registered (chain head 0119; the live registry additionally carries the unregistered 0118) · the fixed purge EF deployed live (401/401 smoke) · **the live verification 15/15 GREEN, zero residue** (`scripts/t-415-live-verification.mjs` — the backup_archives probe round-trip incl. the purge RPC selectivity, the append-only audit tamper test, the sync_queue + mark_processed + upsert-idempotency contract, the six-family FK-orphan + business-key sweep). **Ten problems flipped RESOLVED/TESTED:** BKUP-501/502/503/504/505/506/507 + SYNC-108/109/111. **Full record:** `docs/recovery/t-415-live-verification.md`. **Left (the VERIFIED gate):** the owner's packaged-app UI pass over the changed surfaces (the Settings → Sauvegarde status chips + the restored-mode banner) + the EF's authorized CRON path to exercise on the next Sunday tick + the documented boundaries (the cold-cache backup snapshot; the Supabase-mode restore rehydrates the OFFLINE operating layer — full server rehydration stays the Excel-import path) — see unknowns.

## T-416 — The Purge Button: the canonical student/parent domain reset (issue #12) — TESTED (live-verified, P0)

**Status:** IMPLEMENTED / TESTED — the local suites green (68 tests across 3 suites + the restructured 43-assertion guard suite), migrations 0120 + 0121 applied and registered live, the live verification **22/22 GREEN** (the rollback-safe sandbox 12/12 + the authenticated UI path + the no-interference aftermath). The remaining VERIFIED gate is the owner's real execution from the packaged app (the standing convention — never agent-run against real data).

**Registered:** 2026-09-26 (the 100th session — the owner's issue-#12 mandate, executed on the owner's follow-up directive "Do testing and make sure the purge does not interfere with the sync and backup processes")

**Related:** issue #12 (GitHub) · PURGE-500 / PURGE-501 / PURGE-502 / PURGE-503 · T-415 (the sync/backup hardening this task must not disturb — BKUP-501..507, SYNC-108/109/111) · migration 0100 (the soft-delete RPC house pattern) · ADR-027 (the purge contract) · the unregistered live migration 0118 (the overload-reconciliation hazard, PURGE-500)

### The mandate (the owner's issue-#12 brief, verbatim scope)

Add a **Purge Button** that completely purges student-related data: student records/profiles, associated parent records/profiles, enrollments and academic records, financial records (transactions, payments, payment allocations, debts, credits, balances, installments), student/parent-associated historical records, and any other dependent or derived data — **no orphaned, inconsistent, or partially deleted records** (a FULL student/parent data reset, not a per-profile deletion). Administrative/destructive: appropriate safeguards + confirmation so it cannot be triggered accidentally. The owner's session directive adds the explicit gate: **the purge must not interfere with the sync and backup processes.**

### The dependency census (from the canonical chain + the live-schema authority rule §57c)

**Direct FK families** — `parents` (9 referencing tables): students, parent_student_links, activation_codes, installments, payments, invoices, ledger_entries, account_adjustments, receipts (⚠ live-absent, §57c — guard with an existence check). `students` (10): + grades*, attendance_records*, academic_history*, student_academic_histories* (⚠ the 0004 family predates `students` (0005) — student_id columns with NO FK constraints; soft references only the FK graph cannot see), discount_applications, service_enrollments, student_documents.

**Financial closure order:** payment_allocations (via payment_id/installment_id) → payments → installments → invoices → ledger_entries → account_adjustments → receipts? → discount_applications → service_enrollments.

**Portal/auth closure** (the parents.auth_user_id + students.auth_user_id join): chat_messages/chat_channels (member_ids jsonb), notifications (target_user_id + link_entity polymorph), notification_preferences, device_tokens, calendar_events (target_entity polymorph), sessions, role_assignments, user_profiles, auth.users.

**Sync staging:** sync_queue rows for the five domain entities (PURGE-501) — deleted in the same transaction; everything else in the queue untouched.

**NEVER touched (the no-interference contract):** backup_archives + every backup RPC/EF (BKUP family), the sync infrastructure itself (sync_queue non-domain rows, mark_sync_queue_processed, the push RPCs), audit_logs (append-only forensic journal — §15.26; the purge itself WRITES one audit entry), the academic catalog (academic_years/levels, subjects, classes, class_subjects, filieres, timetable_*, rooms), the workforce/operations domains (personnel*, salary_*, expense_*, suppliers, inventory_*, pending_receipts, purchases, deliveries), workflow engine history (workflow_runs — operational audit, the §15.26 forensic precedent), ai_request_logs, tasks, releve_entries (personnel timesheet), onboarding_states, system_settings.

**Scope boundaries (ADR-027):** the purge is TENANT-WIDE (all parents + students of the caller's tenant) — the issue's "full reset" semantics; account_approval_requests are removed ONLY when target-linked to purged parents/students (unlinked pending signups are pre-parent data, not student/parent-associated); pending_receipts is the OPERATIONS purchase-receipt table (supplier deliveries), NOT the financial receipts family (a naming trap).

### The plan (5 phases)

1. **Phase 0 (this commit):** register T-416 + PURGE-500/501 + ADR-027 (§13 — problems before fixes).
2. **Phase 1 — migration 0120:** ONE canonical `purge_student_parent_domain(p_confirm_phrase text, p_dry_run boolean default true, p_tenant_id uuid default null)` RPC — SECURITY DEFINER, super_admin OR DB-superuser-console gate, typed phrase 'PURGER' required for execute, dry-run default (counts only, deletes nothing), FK-safe ordered deletes over the full census, sync_queue domain cleanup (PURGE-501), backup/audit/catalog untouched, one append-only audit entry, per-table counts in the return; FIRST drops every existing overload of the function name (the live-0118 reconciliation, PURGE-500). Registered atomically (the T-091 block).
3. **Phase 2 — desktop:** the Settings "Zone de danger" card (general tab) — super_admin-gated, Supabase-mode-gated, dry-run preview → typed phrase 'PURGER' → ConfirmModal recap → execute; a `purgeStudentParentDomain` repository method (the 0100 RPC-call pattern).
4. **Phase 3 — local gates:** the SQL source-guard suite (order, gates, the no-interference assertions: no backup_archives delete, no blanket sync_queue delete, audit insert-only, receipts existence-guard, overload-drop present) + the UI suite + the repository contract suite + the FULL vitest/tsc/eslint gates vs the documented baselines.
5. **Phase 4 — live verification (`t-416-purge-live-verification.mjs`):** the transactional BEGIN…ROLLBACK sandbox (seed FAKE-marked probes under the real tenant → execute the REAL RPC → assert zero residue across every family + the no-interference proofs: backup_archives count invariant, a non-domain sync_queue probe survives + mark_sync_queue_processed still works on it, the purge audit entry exists, purge_expired_backups still callable) → ROLLBACK (the real 196 parents / 290 students NEVER touched); + the admin-JWT PostgREST dry-run (the exact UI path); + migration 0120 applied live + registered. Owner executes the real purge from the packaged app when they choose (the VERIFIED gate, the standing visual-acceptance convention).

### The execution record (the 100th session, 2026-09-26 — all phases COMPLETE)

- **Phases 0-3 (commits 6be70b9 / 4787bf5 / 5a02b3b / 9589846):** the registration + migration 0120 + the desktop surface + the 68 local tests + the live script — all pushed (see the prior session's record).
- **Phase 4 (this session, the owner supplied the live credentials):** the eight-iteration live run — every failure a real find (the GoTrue trigger seeding path, the NOT-NULL/CHECK/FK live census, the Management-API `postgres`-is-not-superuser gate behavior, and TWO ship-blocking defects registered and fixed: **PURGE-502** the orphaned approval request, **PURGE-503** the `jsonb_object_length` audit-note landmine — migration **0121** carries both fixes + the PURGE-502 closure). Final: **22/22 GREEN**, sandbox **12/12**, zero residue, the real census untouched (196/290, audit 1088→1088), the dry-run census through the authenticated path exactly matching the baseline (691). Full evidence: `docs/recovery/t-416-live-verification.md`; knowledge: AGENTS.md §15.39; ADR-027 amended (the approval-request boundary refinement + the live record).
- **The owner's no-interference gate — proven at four layers:** the 43 source guards (the FILE cannot reach the backup/sync infrastructure), the sandbox survivors (a non-domain sync_queue row and a backup_archives probe seeded under the real tenant SURVIVED the execute), the aftermath census (the four sync/backup RPCs present; backup_archives 0→0), and the PostgREST gate (service-role refused; only a signed-in super_admin or a DB-superuser console passes).
- **Remaining:** the owner's packaged-app pass (Settings → Zone de danger) and the real execution when they choose it — the card's dry-run will show the same 691-row census.

## T-417 — Excel Import Performance Optimization (issue #19) — IMPLEMENTED / TESTED (P0)

**Registered:** 2026-09-27 (the 101st session — the owner's issue-#19 mandate: "The Excel import process is currently extremely slow, especially when importing larger Excel files... Optimize the importer so that Excel files can be processed significantly faster... Do not simply remove validation, skip important processing, reduce data integrity checks... For every task: implement, test, verify, document, register, commit")

**Related:** issue #19 (GitHub) · PERF-503 (the problem this task closes) · PERF-501/PERF-502 (the interactive-registration round-trip precedent — T-397/T-398) · IMPORT-106/107/108/109 (the empty-state correctness oracle this task must keep green) · IMPORT-110 (bulkImportInstallments' ON CONFLICT history) · T-364 (the flush-idempotency suite whose IMPORT-109 probe now runs through the batch seam) · T-414 (the concurrent session's import-config engine — the sheet-detection rework behind the census drift documented in PERF-503's status note)

### The mandate (the owner's issue-#19 brief, verbatim scope)

Profile and optimize the FULL Excel import pipeline — file reading/parsing, validation, transformation, student/parent processing, relationship resolution, academic + financial data processing, DB reads/writes, duplicate checks, repeated queries, progress tracking — with **real measurements before AND after**, on realistic and larger datasets, **without sacrificing correctness, validation, data integrity, relationships, financial calculations, or existing functionality**. Identify the actual bottlenecks BEFORE changing the implementation. Add regression/performance tests so the importer never becomes extremely slow again.

### The measured baseline (2026-09-27, pre-optimization, the REAL `Suivis clients  2026_2027.xlsx`)

- **Repository call census (instrumented stubs, instant latency):** 636 `parents.search` + 390 `students.search` + 253 `createParent` + 390 `createStudent` + 0 `updateStudent` + 3 bulk flushes = **1,672 repository calls**, of which 1,316 are per-row sequential identity legs.
- **Wall clock under a scaled network profile (25 ms search / 50 ms create / 45 ms update / 100 ms bulk — 1/12th of the real Algeria→eu-west-1 RTT):** **149,090 ms**.
- **Extrapolations:** Supabase mode ≈ 1,286 sequential RTTs (createParent/createStudent each cost RPC + full-row fetch = 2 RTTs) ≈ **10–20 min** at the owner's documented 476–952 ms/RTT route (PERF-501's measured profile); mock mode ≈ 8+ min (123 s of search delays + 129 s of create delays + **235 s of the installment-flush sleep** — see PERF-503's part (c)).

### The plan (3 phases)

1. **Phase 0 (this commit):** register T-417 + PERF-503 (§13 — problems before fixes) with the measured baseline evidence above.
2. **Phase 1 — the implementation:** (a) a batch seam on `StorageAdapter` (`upsertRecordsBatch`, default = the exact legacy sequential loop — every other adapter unchanged); (b) `RepositoryStorageAdapter` overrides it: ONE snapshot read of each identity store (2 search calls, replacing the 1,026 per-row searches) + in-memory multi-pronged identity resolution (the exact IMPORT-109 match semantics, snapshot-served) + a bounded-concurrency pool over INDEPENDENT families (8 default; intra-family row order strictly preserved); (c) the missing `MockInstallmentRepository.bulkImportInstallments` (the IMPORT-108 parity gap — the mock flush slept 120 ms × 1,962 rows); (d) the engine buffers a sheet's validated rows and issues ONE batch call (parse/validate/warnings/rejects/ref-inserts still per-row during iteration — byte-identical); (e) the NEW performance suite `src/tests/performance/t-417-excel-import-perf.test.ts` (census + call-count budget + wall-clock budget + re-import no-op + atomicity + per-row error reporting + bounded-concurrency proof).
3. **Phase 2 — the closeout:** PERF-503 → RESOLVED/TESTED, T-417 → TESTED, change-log, next-task, AGENTS.md §15.58, current-state.

### The execution record (the 101st session, 2026-09-27 — Phases 0-2 COMPLETE)

- **Status:** IMPLEMENTED / TESTED (VERIFIED is gated on the owner's packaged-app import run — the standing visual-acceptance convention; the machine layers are all green).
- **Phase 0 (a8293f0):** T-417 + PERF-503 registered (§13) with the measured pre-optimization baseline: 1,672 repository calls (636 parent searches + 390 student searches + 643 writes), 149,090 ms under the scaled network profile, the 10–20 min owner-route and 8+ min mock-mode extrapolations, the 235 s mock installment-flush sleep.
- **Phase 1 (ed626c2):** the batch seam (`StorageAdapter.upsertRecordsBatch`, default = the exact legacy sequential loop) + the ETAT batch (2 snapshot reads replacing 1,026 searches; the exact IMPORT-109 match semantics served in-memory; an 8-wide bounded-concurrency pool over independent families with intra-family row order strictly preserved; `upsertRecord` = a batch of ONE — a single row-processing implementation) + `MockInstallmentRepository.bulkImportInstallments` (the IMPORT-108-class parity fix) + the engine's one-batch-per-sheet write (parse/validate/warning/reject/ref phases byte-identical; write-phase progress on the sheet:progress channel) + the 6-test performance suite + the census-oracle un-skip (the Excel/ candidates; the 201→202 anchor re-pinned with the stash-verified drift proof) + the t-364 IMPORT-109 probe through the public seam.
- **Separate baseline-repair commit (f02a9b6):** the 22 pre-existing tsc errors in the concurrent session's test files repaired (type-level only) — the "tsc 0" gate restored repo-wide (the 99th-session next-task standing hazard).
...
## T-419 — The Unified Testing Architecture: recover, audit, and unify the three test systems into ONE framework with ONE entry point (issue #22) — IMPLEMENTED / TESTED (P0)
...
- **Phase 5 COMPLETE** (b4e3ab8): `docs/testing/unified-architecture.md` — the architecture map + the complete §38 migration manifest (every artifact accounted) + the correctness-audit record + the TEST-309 fakes census with the consolidation pattern.
