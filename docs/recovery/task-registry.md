...
- **Status:** COMPLETED (TESTED, 2026-09-16 — gates: tsc 0 / eslint 0 errors / 169 files / 3468 tests of which 3 failed = the SAME pre-existing CALC-001 cross-platform failures attributed at the 73rd-session open (+14 new t-384 tests, 0 new failures); the flow is client-driven PostgREST + RLS — no DB change, no EF, no new migration → no separate live leg)
- **Problems:** PARENT-500 (registered BEFORE the fix per §13 and CLOSED TESTED same-session — see problem-registry)
