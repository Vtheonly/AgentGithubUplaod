...
## Progress summary (2026-09-01, updated at the 18th repair session open — owner-mandated batch checked out; chain check 60/60 = 0001–0063, zero drift; baselines re-established: desktop 2215, website 425, Android 298 — all green BEFORE any change)
...
| **In Progress (18th session)** | 10 | **T-024** (Android homework UUID + promotion propagation), **T-039** (Android pull-sync completeness), **T-082** (Android lint gate), **T-046-gap** (Room exportSchema + upgrade test — session-17 follow-up), **DTO-TENANT** (SharedDtoMappers null-tenant pull default — session-17 follow-up), **T-069** (Android realtime subscriptions), **T-102-follow-up** (Android chat read-side + online sends), **T-107-residual** (portal live re-render with the publishable default), **WEAK-030** (expense-approval state-machine migration 0064 + live MIG-TOKENS apply), **WEBSITE-DOC** (AGENTS.md stale realtime claims post-T-032) — selected per next-task.md recommendation, balanced across platforms; session-opening ritual + MIG-TOKENS verification DONE (see change-log SESSION-18-OPEN) |
...
**17th repair session (2026-09-01) — owner-mandated batch in flight.** Session-opening chain check: 60/60 (0001–0063), zero drift (one cosmetic live-label quirk: schema_migrations row version 0050 carries the name label `chat_read_receipts` — versions 1:1 with the repo, statements previously diff-verified; do NOT "fix" by re-registering). Completed so far this session: **T-106 (DESK-LOGIN-200 desktop login fix, VERIFIED live)** and **T-107 (MIG-KEYS-201 new-format API key migration, TESTED + live-verified)**.
