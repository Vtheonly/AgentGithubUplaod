...
| **In Progress (19th session close)** | 2 | **T-069** (Android realtime subscriptions), **T-102-follow-up** (Android chat read-side + online sends) — DEFERRED AGAIN (19th session's context budget went to the owner mandate: live drift 0065 + auth + key consistency); both are full feature builds needing a dedicated session; neither started, nothing half-done. The Android toolchain IS re-provisioned and green this session (JDK 21 at /home/z/my-project/jdk + SDK at /home/z/my-project/android-sdk — recipe in scripts/android-env.sh), so the NEXT session can pick them up immediately |
| **Ready** | 4 | T-024, T-039, T-043, T-044, T-069 (T-017, T-020, T-021, T-034, T-046, T-051, T-104 → TESTED/VERIFIED in the 16th–17th sessions; T-108 + T-106 + T-107 opened and completed as owner-reported) |
